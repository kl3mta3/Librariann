...
RUN /copy_runtime.sh
RUN chmod +x /Librariann/Librariann

#Production image
FROM ubuntu:noble

COPY --from=copytask /Librariann /librariann
COPY --from=copytask /files/wwwroot /librariann/wwwroot
COPY Librariann.Server/config/appsettings.json /tmp/config/appsettings.json
