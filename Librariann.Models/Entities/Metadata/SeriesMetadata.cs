...
public class SeriesMetadata : IHasConcurrencyToken, IHasKPlusMetadata, IHasTags<Tag>
{
    ...
    /// <summary>
    /// Are all instances of the role from Librariann+
    /// </summary>
    /// <param name="role"></param>
    /// <returns></returns>
    public bool AllLibrariannPlus(PersonRole role)
    {
        var people = People.Where(p => p.Role == role);
        if (people.Any()) return people.All(p => p.LibrariannPlusConnection);
        return false;
    }
