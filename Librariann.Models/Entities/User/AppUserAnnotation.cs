...
public class AppUserAnnotation : IEntityDate
{
    ...
    /// <summary>
    /// EPUB Canonical Fragment Identifier for this highlight's position, used by the foliate-js based reader.
    /// A single CFI range string covers both single- and multi-element highlights (unlike <see cref="XPath"/>/
    /// <see cref="EndingXPath"/>, which needed two separate anchors for that). Null for annotations created
    /// before this reader migration until they're lazily backfilled (see the migration plan) - <see cref="XPath"/>/
    /// <see cref="EndingXPath"/> are kept indefinitely as the source of truth for that backfill, not replaced.
    /// </summary>
