...
/// <summary>
/// Explicit replacement for the old AutoMapper <c>ServerSettingConverter</c> (<c>ITypeConverter</c>). Logic is
/// unchanged from the original converter; this just drops the AutoMapper wrapper interface.
/// </summary>
public static class ServerSettingMapping
{
    public static ServerSettingDto ToServerSettingDto(this IEnumerable<ServerSetting> source)
    {
        var destination = new ServerSettingDto();
        foreach (var row in source)
        {
            switch (row.Key)
            {
                ...
                case ServerSettingKey.EnableFolderWatching:
                    ...
                    break;
                case ServerSettingKey.WriteMetadataToFiles:
                    destination.WriteMetadataToFiles = bool.Parse(row.Value);
                    break;
                ...
                case ServerSettingKey.OidcConfiguration:
                    ...
                    break;
                case ServerSettingKey.FfmpegPath:
                    destination.FfmpegPath = row.Value;
                    break;
                case ServerSettingKey.MetadataProviderContactEmail:
                    destination.MetadataProviderContactEmail = row.Value;
                    break;
                case ServerSettingKey.KokoroExecutablePath:
                    destination.KokoroExecutablePath = row.Value;
                    break;
                case ServerSettingKey.KokoroUseGpu:
                    destination.KokoroUseGpu = bool.Parse(row.Value);
                    break;
                case ServerSettingKey.KokoroSyncFfmpegPath:
                    destination.KokoroSyncFfmpegPath = bool.Parse(row.Value);
                    break;
                case ServerSettingKey.ShowRequestInviteLink:
                    destination.ShowRequestInviteLink = bool.Parse(row.Value);
                    break;
                case ServerSettingKey.AutoAcceptInviteRequests:
                    destination.AutoAcceptInviteRequests = bool.Parse(row.Value);
                    break;
                case ServerSettingKey.DefaultInvitePermissions:
                    destination.DefaultInvitePermissions = JsonSerializer.Deserialize<DefaultInvitePermissionsDto>(row.Value)!;
                    break;
                case ServerSettingKey.KokoroEndpointUrl:
                    destination.KokoroEndpointUrl = row.Value;
                    break;
