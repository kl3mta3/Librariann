...
using Librariann.Models.DTOs.OPDS;
using Librariann.Models.DTOs.OPDS.Requests;
    ...
    : BaseApiController
{
    ...
    /// <summary>
    /// Returns the Catalogue for Librariann's OPDS Service
    /// </summary>
    ...
    public async Task<ActionResult> DownloadFile(string apiKey, int seriesId, int volumeId, int chapterId, string filename)
    {
        ...
        var outputPath = Path.Join(directoryService.TempDirectory, $"librariann_opds_merged_c{chapterId}_{dateStr}.cbz");
        ...
        [FromQuery] int volumeId,[FromQuery] int chapterId, [FromQuery] int pageNumber, [FromQuery] bool saveProgress = true)
    {
        ...
        try
        {
            ...
            {
                // Librariann expects 0-N for progress, KOReader doesn't respect the OPDS-PS spec and does some wierd stuff
                // https://github.com/Kareadita/Librariann/pull/4014#issuecomment-3313677492
