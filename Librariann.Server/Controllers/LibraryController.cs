...
using Librariann.Models.DTOs.JumpBar;
using Librariann.Models.DTOs.SignalR;
using Librariann.Models.DTOs.System;
    ...
    : BaseApiController
{
    ...
    public async Task<ActionResult<LibraryDto?>> AddLibrary(UpdateLibraryDto dto)
    {
        ...
        // I wasn't able to reproduce but could validate it didn't happen with this extra commit. (https://github.com/Kareadita/Librariann/issues/4248)
    ...
    public ActionResult<IEnumerable<DirectoryDto>> GetDirectories(string? path)
    {
        ...
        {
            Logger.LogWarning("Library directory picker could not access {DirectoryPath}", path);
            return StatusCode(StatusCodes.Status403Forbidden, new
            {
                message = "Librariann cannot access this folder. Choose a media folder the server account can read."
            });
        }
        catch (IOException exception)
        {
            Logger.LogWarning(exception, "Library directory picker could not enumerate {DirectoryPath}", path);
            return StatusCode(StatusCodes.Status409Conflict, new
            {
                message = "Librariann could not read this folder. Check that it is available and try again."
            });
        }
    }
    ...
    /// <summary>
    /// Given a valid path, will invoke either a Scan Series or Scan Library. If the folder does not exist within Librariann, the request will be ignored
    /// </summary>
    /// <param name="dto"></param>
    /// <returns></returns>
    [AllowAnonymous]
    ...
    private async Task<bool> DeleteLibrary(int libraryId, int userId, CancellationToken ct = default)
    {
        ...
        try
        {
            if (TaskScheduler.HasScanTaskRunningForLibrary(libraryId))
            {
                ...
                throw new LibrariannException(await localizationService.TranslateAsync(userId, "delete-library-while-scan"));
            }

            var library = await unitOfWork.LibraryRepository.GetLibraryForIdAsync(libraryId, ct: ct);
            if (library == null)
            {
                throw new LibrariannException(await localizationService.TranslateAsync(userId, "library-doesnt-exist"));
            }
    ...
    {
        if (LibrariannPlusConfiguration.MetadataProvidersForLibraryTypes.TryGetValue(libraryType, out var providers))
        {
            return Ok(providers);
        }
    ...
    /// <summary>
    /// <see cref="LibraryType"/> with LibrariannPlus metadata support
    /// </summary>
    /// <returns></returns>
    [HttpGet("metadata-enabled-libraries")]
    public ActionResult<List<LibraryType>> LibrariesWithMetadataSupport()
    {
        return Ok(LibrariannPlusConfiguration.MetadataProvidersForLibraryTypes.Keys.ToList());
    }

    /// <summary>
    /// <see cref="LibraryType"/> with LibrariannPlus scrobble support
    /// </summary>
    /// <returns></returns>
    [HttpGet("scrobble-enabled-libraries")]
    public ActionResult<List<LibraryType>> LibrariesWithScrobbleSupport()
    {
        return Ok(LibrariannPlusConfiguration.ScrobbleProvidersForLibraryTypes.Keys.ToList());
    }
    ...
    {
        if (!LibrariannPlusConfiguration.IsValidMetadataProviderForLibraryType(type, provider))
        {
            throw new LibrariannException("invalid-metadata-provider");
        }
    }
