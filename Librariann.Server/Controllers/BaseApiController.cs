...
public class BaseApiController : ControllerBase
{
    ...
    /// </returns>
    protected ActionResult CachedFile(string? path, int maxAge = 300, string? contentTypeOverride = null)
    {
        ...
        // MimeTypeMap doesn't recognize every extension (notably .m4b) - callers serving a format it can get
        // wrong/blank on should pass an explicit contentTypeOverride rather than relying on the generic lookup.
        var contentType = contentTypeOverride ?? MimeTypeMap.GetMimeType(Path.GetExtension(path));
