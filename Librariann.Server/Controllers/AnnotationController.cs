    ...
    : BaseApiController
{
    ...
    /// <summary>
    /// Backfills the CFI position for an existing annotation, computed client-side by the v2 (foliate-js) reader
    /// from the annotation's stored XPath/EndingXPath/SelectedText - see the reader migration plan. Separate from
    /// <see cref="UpdateAnnotation"/> since this isn't user-editable content, just a one-time positional backfill.
    /// </summary>
    /// <param name="annotationId"></param>
    /// <param name="dto"></param>
    [HttpPost("{annotationId}/cfi")]
    [DisallowRole(PolicyConstants.ReadOnlyRole)]
    public async Task<ActionResult> SetAnnotationCfi(int annotationId, SetAnnotationCfiDto dto)
    {
        try
        {
            await annotationService.SetAnnotationCfi(UserId, annotationId, dto.Cfi);
            return Ok();
        }
        catch (LibrariannException ex)
        {
