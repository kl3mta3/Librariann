    ...
    : BaseApiController
{
    ...
    public async Task<ActionResult<UserDto>> Authenticate([Required] string apiKey, [Required] string pluginName)
    {
        ...
        if (userId <= 0)
        {
            ...
            throw new LibrariannUnauthenticatedUserException();
        }
        ...
        return new UserDto
        {
            ...
            ApiKey = apiKey,
            LibrariannVersion = (await unitOfWork.SettingsRepository.GetSettingAsync(ServerSettingKey.InstallVersion)).Value
        };
    }

    /// <summary>
    /// Returns the version of the Librariann install
    /// </summary>
    /// <remarks>This will log unauthorized requests to Security log</remarks>
    ...
    {
        var userId = await unitOfWork.UserRepository.GetUserIdByAuthKeyAsync(apiKey);
        if (userId <= 0) throw new LibrariannUnauthenticatedUserException();
