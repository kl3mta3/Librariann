...
public static class ClaimsPrincipalExtensions
{
    ...
    {
        var userClaim = user.FindFirst(JwtRegisteredClaimNames.Name) ?? throw new LibrariannException(NotAuthenticatedMessage);
        return userClaim.Value;
    }

    public static int GetUserId(this ClaimsPrincipal user)
    {
        var userClaim = user.FindFirst(ClaimTypes.NameIdentifier) ?? throw new LibrariannException(NotAuthenticatedMessage);
