    ...
    : IAnnotationService
{
    ...
    /// <param name="ct"></param>
    /// <returns></returns>
    /// <exception cref="LibrariannException">Message is not localized</exception>
    public async Task<AnnotationDto> CreateAnnotation(int userId, AnnotationDto dto, CancellationToken ct = default)
    {
        try
        {
            ...
            {
                throw new LibrariannException("invalid-payload");
            }

            var chapter = await unitOfWork.ChapterRepository.GetChapterAsync(dto.ChapterId, ct: ct) ?? throw new LibrariannException("chapter-doesnt-exist");
            ...
            var annotation = new AppUserAnnotation()
            {
                ...
                // Highlights created by the foliate-js reader carry a real Cfi and an empty XPath (the reverse
                // of a pre-migration annotation) - both fields just get copied through as given either way.
                Cfi = dto.Cfi,
        ...
        catch (Exception ex)
        {
            ...
            throw new LibrariannException("annotation-failed-create");
        }
    }
    ...
    public async Task<AnnotationDto> UpdateAnnotation(int userId, AnnotationDto dto, CancellationToken ct = default)
    {
        try
        {
            ...
            if (annotation == null || annotation.AppUserId != userId) throw new LibrariannException("denied");
    ...
    public async Task SetAnnotationCfi(int userId, int annotationId, string cfi, CancellationToken ct = default)
    {
        if (string.IsNullOrWhiteSpace(cfi)) throw new LibrariannException("invalid-payload");

        var annotation = await unitOfWork.AnnotationRepository.GetAnnotation(annotationId, ct);
        if (annotation == null || annotation.AppUserId != userId) throw new LibrariannException("denied");

        // Idempotent by design - the reader calls this opportunistically on every book open until an
        // annotation has a Cfi, so a second call racing the first (e.g. two tabs) just re-sets the same value.
        annotation.Cfi = cfi;
        unitOfWork.AnnotationRepository.Update(annotation);
        ...
        CancellationToken ct = default)
    {
                        ...
                        .Select(volumeGroup => new
                        {
                            ...
                            annotations = volumeGroup.Select(annotation =>
                            {
                                ...
                                return new
                                {
                                    ...
                                    obsidianTags = new[] { "#librariann", $"#{seriesGroup.Key.SeriesName.ToLowerInvariant().Replace(" ", "-")}", "#highlights" },
                                    obsidianTitle,
        ...
        catch (Exception ex)
        {
            ...
            throw new LibrariannException("annotation-export-failed");
        }
    }
