...
using Librariann.Models.DTOs.ReadingLists.Request;
    ...
    : IReadingListService
{
    ...
    public async Task<ReadingList> CreateReadingListForUser(AppUser userWithReadingList, string title)
    {
        ...
        if (hasExisting)
        {
            throw new LibrariannException("reading-list-name-exists");
        }

        var readingList = new ReadingListBuilder(title).Build();
        userWithReadingList.ReadingLists.Add(readingList);

        if (!unitOfWork.HasChanges()) throw new LibrariannException("generic-reading-list-create");
    ...
    public async Task UpdateReadingList(ReadingList readingList, UpdateReadingListDto dto)
    {
        ...
        if (string.IsNullOrEmpty(dto.Title)) throw new LibrariannException("reading-list-title-required");
    ...
    /// <summary>
    /// Removes the Reading List from librariann
    /// </summary>
    /// <param name="readingListId"></param>
