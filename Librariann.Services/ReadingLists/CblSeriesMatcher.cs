...
using Librariann.Models.DTOs.ReadingLists.CBL;
using Librariann.Models.DTOs.ReadingLists.CBL.Import;
using Librariann.Models.DTOs.ReadingLists.CBL.Internal;
...
namespace Librariann.Services.ReadingLists;

/// <summary>
/// Result of matching a single CBL item to Librariann entities
/// </summary>
...
internal static class CblSeriesMatcher
{
        ...
        IList<Chapter> alternateSeriesChapters)
    {
        ...
        var externalIdByLibrariannId = externalIdChapters
        ...
        foreach (var item in items)
        {
            ...
            // Tier 1: External IDs
            if (TryMatchByExternalId(item, externalIdByLibrariannId, externalIdByComicVine, externalIdByMetron, out var extMatch, out var extChapter))
            {
    ...
    private static bool TryMatchByExternalId(ParsedCblItem item,
        Dictionary<int, Chapter> byLibrariannId,
        ...
        out MatchedItem match, out Chapter matchedChapter)
    {
        ...
        {
            if (extId.Provider == CblExternalDbProvider.Librariann
                && int.TryParse(extId.IssueId, out var librariannChapterId))
            {
                if (byLibrariannId.TryGetValue(librariannChapterId, out var ch))
                {
