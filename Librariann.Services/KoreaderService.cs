    ...
    : IKoreaderService
{
    ...
    public async Task SaveProgress(KoreaderBookDto koreaderBookDto, int userId, CancellationToken ct = default)
    {
        ...
        if (file == null)
        {
            ...
            throw new LibrariannException(await localizationService.TranslateAsync(userId, "file-missing"));
        }
        ...
        if (userProgressDto == null)
        {
            ...
            if (chapterDto == null) throw new LibrariannException(await localizationService.TranslateAsync(userId, "chapter-doesnt-exist"));

            var volumeDto = await unitOfWork.VolumeRepository.GetVolumeByIdAsync(chapterDto.VolumeId, ct: ct);
            if (volumeDto == null) throw new LibrariannException(await localizationService.TranslateAsync(userId, "volume-doesnt-exist"));

            var seriesDto = await unitOfWork.SeriesRepository.GetSeriesDtoByIdAsync(volumeDto.SeriesId, userId, ct);
            if (seriesDto == null) throw new LibrariannException(await localizationService.TranslateAsync(userId, "series-doesnt-exist"));
        ...
        // Normal saving from librariann will be //body/h2[1]
