...
using Librariann.Models.DTOs.LibrariannPlus.Account;
...
public class ScrobblingService : IScrobblingService
{
    ...
    private const string InvalidKPlusLicenseErrorMessage = "Librariann+ subscription no longer active";
    ...
    public ScrobblingService(IUnitOfWork unitOfWork, IEventHub eventHub, ILogger<ScrobblingService> logger,
        ...
        ILibrariannPlusApiService librariannPlusApiService, IServiceProvider serviceProvider, ILibrariannPlusAuditService auditService,
    ...
    public async Task CheckExternalAccessTokens(CancellationToken ct = default)
    {
        ...
        foreach (var user in users)
        {
            foreach (var (provider, settings) in user.ScrobbleProviders)
            {
                ...
                var canBeRefreshedByLibrariann = provider.SupportsOAuthTokenRefresh()
                    && !string.IsNullOrEmpty(settings.RefreshToken);

                // Send early reminder 5 days before token expiry. Unless Librariann can refresh the token for the user
                if (!canBeRefreshedByLibrariann && await ShouldSendEarlyReminder(user.Id, tokenExpiry))
                {
    ...
    private async Task<bool> HasTokenExpired(string token, ScrobbleProvider provider)
    {
        ...
        var result = await _librariannPlusApiService.HasTokenExpiredForProviderAsync(provider, token, license.Value);
        if (!result.IsSuccess)
        {
    ...
    {
        return LibrariannPlusConfiguration.ScrobbleProvidersForLibraryTypes.TryGetValue(libraryType, out var providers) && providers.Contains(provider);
    }
        ...
        Series series)
    {
        ...
        var providerCandidates = scrobbleProviders ?? LibrariannPlusConfiguration.AllInUseScrobbleProviders;
    ...
    private async Task<ScrobbleUpdateContext> CreateScrobbleUpdateContext(int userId, int seriesId, int? chapterId, bool isBackfill, CancellationToken ct)
    {
        ...
        if (series == null) throw new LibrariannException(await _localizationService.TranslateAsync(userId, "series-doesnt-exist"));

        Chapter? chapter = null;
        if (chapterId.HasValue)
        {
            chapter = await _unitOfWork.ChapterRepository.GetChapterAsync(chapterId.Value, ChapterIncludes.Volumes, ct: ct);
            if (chapter == null) throw new LibrariannException(await _localizationService.TranslateAsync(userId, "chapter-doesnt-exist"));
        }

        var user = await _unitOfWork.UserRepository.GetUserByIdAsync(userId, AppUserIncludes.UserPreferences, ct);
        if (user == null) throw new LibrariannException(await _localizationService.TranslateAsync(userId, "user-doesnt-exist"));
    ...
    public async Task ScrobbleReadingUpdateForVolume(int userId, int volumeId, CancellationToken ct = default)
    {
        ...
        if (volume == null) throw new LibrariannException(await _localizationService.TranslateAsync(userId, "volume-doesnt-exist"));
    ...
    private async Task<bool> CheckIfCannotScrobble(ScrobbleProvider provider, ScrobbleEventType eventType, int userId, int seriesId, Series series)
    {
        ...
        if (series.DontMatch)
        {
            ...
            await _auditService.LogTemperedAsync(al => al.SeriesId == seriesId && al.UserId == userId, LibrariannPlusAuditCategory.Scrobble,
                LibrariannPlusEventType.ScrobbleEventSkipped, AuditStatus.Info, AuditSubjectType.Series, seriesId: seriesId, payload: new AuditLogScrobbleParamsDto() {Provider = provider, ScrobbleEventType = eventType}, error: "series-dont-match", userId: userId);
            return true;
        }
        ...
        if (await _unitOfWork.UserRepository.HasHoldOnSeries(userId, seriesId))
        {
            ...
            await _auditService.LogTemperedAsync(al => al.SeriesId == seriesId && al.UserId == userId, LibrariannPlusAuditCategory.Scrobble,
                LibrariannPlusEventType.ScrobbleEventSkipped, AuditStatus.Info, AuditSubjectType.Series, seriesId: seriesId, payload: new AuditLogScrobbleParamsDto() {Provider = provider, ScrobbleEventType = eventType}, error: "scrobble-hold-active", userId: userId);
            return true;
        }
        ...
        {
            await _auditService.LogTemperedAsync(al => al.SeriesId == seriesId && al.UserId == userId, LibrariannPlusAuditCategory.Scrobble,
                LibrariannPlusEventType.ScrobbleEventSkipped, AuditStatus.Info, AuditSubjectType.Series, seriesId: seriesId, payload: new AuditLogScrobbleParamsDto() {Provider = provider, ScrobbleEventType = eventType}, error: "library-scrobbling-disabled", userId: userId);
            return true;
        }
    ...
    private async Task<int> GetRateLimit(ScrobbleProvider provider, string license, string accessToken, CancellationToken ct)
    {
        ...
        var result = await _librariannPlusApiService.GetRateLimitForProviderAsync(provider, accessToken, license, ct);
    ...
    private async Task<ScrobbleSyncContext> PrepareScrobbleContext(CancellationToken ct)
    {
        ...
        return new ScrobbleSyncContext
        {
            ...
            RateProfiles = LibrariannPlusConfiguration.AllInUseScrobbleProviders.ToDictionary(
                p => p,
    ...
    public async Task ProcessUpdatesSinceLastSync(CancellationToken ct = default)
    {
        ...
        {
            _logger.LogError(ex, "Librariann+ API or a Scrobble service may be experiencing an outage. Stopping sending data");
            return;
        }
    ...
    private async Task<bool> ValidateUserToken(AppUser user, ScrobbleEvent evt)
    {
        try
        {
            ...
            await LogScrobbleEvent(evt, LibrariannPlusEventType.ScrobbleEventFailed, AuditStatus.Failure, "token-expired", scrobbleError);
        }
        catch (Exception ex)
        {
    ...
    private async Task<bool> ValidateSeriesCanBeScrobbled(ScrobbleEvent evt)
    {
        ...
        await LogScrobbleEvent(evt, LibrariannPlusEventType.ScrobbleEventFailed, AuditStatus.Failure, "unknown-series", scrobbleError);
        return false;
    }
    ...
    private async Task ProcessEvents(IEnumerable<ScrobbleEvent> events, ScrobbleSyncContext ctx, Func<ScrobbleEvent, Task<ScrobbleV3Dto>> createEvent, CancellationToken ct)
    {
        ...
        foreach (var evt in eventList.Where(e => !CanProcessScrobbleEvent(e)))
        {
            ...
            {
                await _auditService.LogTemperedAsync(al => al.SeriesId == evt.SeriesId, LibrariannPlusAuditCategory.Scrobble,
                    LibrariannPlusEventType.ScrobbleEventSkipped, AuditStatus.Info, AuditSubjectType.Series, evt.SeriesId, payload: ToAuditParams(evt), userId: evt.AppUserId, ct: ct);
            }
            else
            {
                await _auditService.LogTemperedAsync(al => al.SeriesId == evt.SeriesId && al.SubjectId == evt.ChapterId, LibrariannPlusAuditCategory.Scrobble,
                    LibrariannPlusEventType.ScrobbleEventSkipped, AuditStatus.Info, AuditSubjectType.Chapter, evt.SeriesId, subjectId: evt.ChapterId , payload: ToAuditParams(evt), userId: evt.AppUserId, ct: ct);
            }
        }
        ...
        foreach (var evt in eventList.Where(CanProcessScrobbleEvent))
        {
            ...
            if (!gate.HasRateLeft())
            {
                ...
                await LogScrobbleEvent(evt, LibrariannPlusEventType.ScrobbleEventSkipped, AuditStatus.Failure, "rate-limit-hit", ct: ct);
                continue;
            }
    ...
    /// <returns></returns>
    /// <exception cref="LibrariannException">Exceptions may be rethrown as a LibrariannException</exception>
    ...
    public async Task<int> PostScrobbleUpdate(ScrobbleV3Dto data, string license, ScrobbleEvent evt)
    {
        try
        {
            var response = await _librariannPlusApiService.PostScrobbleV3UpdateAsync(data, license);
            ...
            if (response.Successful || response.ErrorMessage == null)
            {
                ...
                await LogScrobbleEvent(evt, LibrariannPlusEventType.ScrobbleEventSent, AuditStatus.Success);
            ...
            if (response.ErrorMessage.Contains("Too Many Requests"))
            {
                ...
                await _auditService.LogAsync(LibrariannPlusAuditCategory.Scrobble, LibrariannPlusEventType.ScrobbleRateLimitHit,
                    AuditStatus.Failure, error: "rate-limit-hit");

                throw new LibrariannException(RateLimitHitErrorMessage);
            }

            if (response.ErrorMessage.Contains("Unauthorized"))
            {
                _logger.LogCritical("Librariann+ responded with Unauthorized. Please check your subscription");
                await _licenseService.HasActiveLicense(true);
                evt.SetErrorMessage(InvalidKPlusLicenseErrorMessage);
                throw new LibrariannException("Librariann+ responded with Unauthorized. Please check your subscription");
            }
            ...
            if (response.ErrorMessage.Contains("Access token is invalid"))
            {
                ...
                await LogScrobbleEvent(evt, LibrariannPlusEventType.ScrobbleEventFailed, AuditStatus.Failure, "invalid-token", scrobbleError);

                throw new LibrariannException("Access token is invalid");
            }

            if (response.ErrorMessage.IsUnknownSeriesError())
            {
                // Log the Series name and Id in ScrobbleErrors
                _logger.LogInformation("Librariann+ was unable to match the series: {SeriesName}", evt.Series.Name);
            ...
            else
            {
                ...
                await LogScrobbleEvent(evt, LibrariannPlusEventType.ScrobbleEventFailed, AuditStatus.Failure, errorMessage, scrobbleError);
            }

            throw new LibrariannException(response.ErrorMessage);
        }
        ...
        #pragma warning restore S2139
        {
            ...
            _logger.LogError(ex, "Scrobbling to Librariann+ API failed due to error: {ErrorMessage}", ex.Message);
            if (ex.StatusCode == 500 || ex.Message.Contains("Call failed with status code 500 (Internal Server Error)"))
            {
                ...
                throw new LibrariannException(BadPayLoadErrorMessage);
            }
            throw;
        }
    }
    ...
    public async Task SyncProviderInfo(int userId, ScrobbleProvider provider, CancellationToken ct = default)
    {
        ...
        var userInfo = await _librariannPlusApiService.GetUserInfo(provider, scrobbleProviderSettings.AuthenticationToken, license, ct);
        if (!userInfo.IsSuccess)
        {
            ...
            await _auditService.LogAsync(LibrariannPlusAuditCategory.System, LibrariannPlusEventType.SystemProviderInfoSync,
    ...
    private async Task<DateTime?> GetTokenExpiry(ScrobbleProvider provider, int userId, string token, CancellationToken ct = default)
    {
        ...
        var tokenExp = await _librariannPlusApiService.GetTokenExpiry(upstream.Value, token, ct);
    ...
    public async Task<bool> RetryScrobbleAsync(int authUserId, LibrariannPlusAuditEntryDto auditEntry, CancellationToken ct = default)
    {
        ...
        await _unitOfWork.LibrariannPlusAuditRepository.MarkAsRetriedAsync(auditEntry.Id, ct);
        return true;
    }
    ...
    private static bool CanProcessScrobbleEvent(ScrobbleEvent readEvent)
    {
        ...
        return LibrariannPlusConfiguration.ScrobbleProvidersForLibraryTypes.TryGetValue(readEvent.Series.Library.Type, out var set)
    ...
    /// <param name="ct"></param>
    private async Task LogScrobbleEvent(ScrobbleEvent evt, LibrariannPlusEventType eventType, AuditStatus status, string? error = null, ScrobbleError? scrobbleError = null, CancellationToken ct = default)
    {
        if (evt.ChapterId is null)
        {
