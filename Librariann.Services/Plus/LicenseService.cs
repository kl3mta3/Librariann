...
public class LicenseService(
    ...
    IVersionUpdaterService versionUpdaterService, ILibrariannPlusApiService librariannPlusApiService,
    ...
    : ILicenseService
{
    ...
    private async Task<bool> IsLicenseValid(string license)
    {
        ...
        try
        {
            var response = await (Configuration.LibrariannPlusApiUrl + "/api/license/check")
    ...
    private async Task<RegisterLicenseResponseDto> RegisterLicense(string license, string email, string? discordId)
    {
        if (string.IsNullOrWhiteSpace(license) || string.IsNullOrWhiteSpace(email))
        {
            return new RegisterLicenseResponseDto()
            {
                ...
                ErrorCode = LibrariannPlusRegistrationErrorCode.RegistrationFailed,
                Successful = false
            };
        }
        ...
        try
        {
            var response = await (Configuration.LibrariannPlusApiUrl + "/api/license/register")
                .WithLibrariannPlusHeaders(license)
            ...
            logger.LogError("Librariann+ registration failed. Code: {Code}, Message: {Message}", response.ErrorCode, response.ErrorMessage);
            return response;
        }
        catch (FlurlHttpException e)
        {
            logger.LogError(e, "Network error reaching Librariann+ API");
            return new RegisterLicenseResponseDto()
            {
                EncryptedLicense = string.Empty,
                ErrorCode = LibrariannPlusRegistrationErrorCode.InternalError,
                Successful = false
            };
        }
    }
    ...
    public async Task<bool> HasActiveLicense(bool forceCheck = false, CancellationToken ct = default)
    {
        ...
        {
            logger.LogError(ex, "There was an issue connecting to Librariann+");
        }
        finally
        {
    ...
    public async Task<bool> HasActiveSubscription(string? license, CancellationToken ct = default)
    {
        ...
        try
        {
            var response = await (Configuration.LibrariannPlusApiUrl + "/api/license/check-sub")
                .WithLibrariannPlusHeaders(license)
                .PostJsonAsync(new LicenseValidDto()
                {
                    License = license,
    ...
    public async Task<LibrariannPlusRegisterResultDto> AddLicense(string license, string email, string? discordId, CancellationToken ct = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(response.EncryptedLicense) || !response.Successful)
            return new LibrariannPlusRegisterResultDto { Success = false, ErrorCode = response.ErrorCode };
        ...
        return new LibrariannPlusRegisterResultDto { Success = true, IsSubscriptionActive = response.IsSubscriptionActive};
    }


    /// <summary>
    /// Removes this installId from Librariann+, essentially allowing the user to re-register an instance
    /// </summary>
    /// <param name="license"></param>
    ...
    public async Task<bool> ResetLicense(string license, string email, CancellationToken ct = default)
    {
        try
        {
            ...
            var response = await (Configuration.LibrariannPlusApiUrl + "/api/license/reset")
                .WithLibrariannPlusHeaders(encryptedLicense.Value)
            ...
            logger.LogError("An error happened during the request to Librariann+ API: {ErrorMessage}", response);
            throw new LibrariannException(response);
        }
        catch (FlurlHttpException e)
        {
            logger.LogError(e, "An error happened during the request to Librariann+ API");
    ...
    /// <summary>
    /// Fetches information about the license from Librariann+. If there is no license or an exception, will return null and can be assumed it is not active
    /// </summary>
    ...
    public async Task<LicenseInfoDto?> GetLicenseInfo(bool forceCheck = false, CancellationToken ct = default)
    {
        ...
        try
        {
            var response = await librariannPlusApiService.GetLicenseInfo(ct);
    ...
    public async Task<bool> ResendWelcomeEmail(CancellationToken ct = default)
    {
        try
        {
            ...
            var httpResponse = await (Configuration.LibrariannPlusApiUrl + "/api/license/resend-welcome-email")
                .WithLibrariannPlusHeaders(encryptedLicense.Value)
    ...
    public async Task<LibrariannPlusLicenseUsageDto> GetLicenseUsage(CancellationToken ct = default)
    {
        ...
        var result = await fileCacheService.GetOrFetchAsync<LibrariannPlusLicenseUsageDto>(
            LicenseUsageCacheKey,
            FileCacheService.LibrariannPlusCacheDirectory,
            ttl,
            async _ => await librariannPlusApiService.GetLicenseUsage(ct),
            shouldCache: r => r?.Stats?.Count > 0,
            ct: ct);

        return result ?? new LibrariannPlusLicenseUsageDto
        {
    ...
    public async Task<bool> CancelLicense(CancelLibrariannPlusLicenseDto dto, CancellationToken ct)
    {
        return await librariannPlusApiService.CancelLicenseAsync(dto, ct);
    }

    public async Task<IList<LibrariannPlusProductInfoDto>> GetProducts(CancellationToken ct = default)
    {
        return await librariannPlusApiService.GetProducts(ct);
    }

    public async Task<string?> RenewLicense(RenewLibrariannPlusLicenseDto dto, CancellationToken ct)
    {
        return await librariannPlusApiService.RenewLicenseAsync(dto, ct);
    }

    public async Task<bool> ChangeEmail(ChangeEmailOnLicenseDto dto, CancellationToken ct)
    {
        return await librariannPlusApiService.ChangeEmail(dto, ct);
    }
    ...
    public async Task LinkDiscord(string discordId, string discordUsername, CancellationToken ct = default)
    {
        ...
        try
        {
            var response = await librariannPlusApiService.LinkDiscord(new LinkDiscordRequestDto
            {
                DiscordId = discordId,
