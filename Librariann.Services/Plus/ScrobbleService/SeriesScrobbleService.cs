...
/// </summary>
public abstract class SeriesScrobbleService<T>(ILogger<T> logger, IUnitOfWork unitOfWork, ILibrariannPlusAuditService auditService): ISeriesScrobbleService
where T: IScrobbleProviderService
{
    ...
    private async Task<bool> ValidateRequiredIds(ScrobbleUpdateContext ctx, ScrobbleEventType eventType, CancellationToken ct)
    {
        ...
        await auditService.LogTemperedAsync(al => al.SeriesId == ctx.Series.Id && al.UserId == ctx.User.Id, LibrariannPlusAuditCategory.Scrobble,
            LibrariannPlusEventType.ScrobbleEventSkipped, AuditStatus.Failure, AuditSubjectType.Series,
        ...
        TransitionRuleKind? ruleKind = null, string? ruleHash = null, CancellationToken ct = default)
    {
        ...
        if (existingEvent is { IsProcessed: false })
        {
            ...
            await auditService.LogScrobbleAsync(LibrariannPlusEventType.ScrobbleEventUpdated, ctx.Series.Id,
        ...
        await auditService.LogScrobbleAsync(LibrariannPlusEventType.ScrobbleEventCreated, ctx.Series.Id,
            new AuditLogScrobbleParamsDto
            {
                Provider = Provider,
                ScrobbleEventType = ScrobbleEventType.ReadStatusUpdate,
