...
/// </summary>
public abstract class ChapterScrobbleService<T>(ILogger<T> logger, IUnitOfWork unitOfWork, ILibrariannPlusAuditService auditService): IScrobbleProviderService
where T: IScrobbleProviderService
{
    ...
    private async Task<bool> ValidateRequiredIds(ScrobbleUpdateContext ctx, ScrobbleEventType eventType, CancellationToken ct)
    {
        ...
        await auditService.LogTemperedAsync(al => al.SubjectId == ctx.Chapter!.Id && al.UserId == ctx.User.Id, LibrariannPlusAuditCategory.Scrobble,
            LibrariannPlusEventType.ScrobbleEventSkipped, AuditStatus.Failure, AuditSubjectType.Chapter,
        ...
        TransitionRuleKind? ruleKind = null, string? ruleHash = null, CancellationToken ct = default)
    {
        ...
        if (existingEvent is { IsProcessed: false })
        {
            ...
            await auditService.LogChapterScrobbleAsync(LibrariannPlusEventType.ScrobbleEventUpdated, ctx.Series.Id, ctx.Chapter.Id,
        ...
        await auditService.LogChapterScrobbleAsync(LibrariannPlusEventType.ScrobbleEventCreated, ctx.Series.Id, ctx.Chapter.Id,
            new AuditLogScrobbleParamsDto
            {
                Provider = Provider,
                ScrobbleEventType = ScrobbleEventType.ReadStatusUpdate,
