...
public class ExternalMetadataService : IExternalMetadataService
{
    ...
    private readonly ILibrariannPlusApiService _librariannPlusApiService;
    private readonly IFileCacheService _fileCacheService;
    private readonly ILibrariannPlusAuditService _auditService;
    ...
    public ExternalMetadataService(IUnitOfWork unitOfWork, ILogger<ExternalMetadataService> logger,
        ...
        ILibrariannPlusApiService librariannPlusApiService, IFileCacheService fileCacheService, ILibrariannPlusAuditService auditService)
    {
        ...
        _librariannPlusApiService = librariannPlusApiService;
        _fileCacheService = fileCacheService;
        _auditService = auditService;

        FlurlConfiguration.ConfigureClientForUrl(Configuration.LibrariannPlusApiUrl);
    }

    /// <summary>
    /// Checks if the library type is allowed to interact with Librariann+
    /// </summary>
    /// <param name="type"></param>
    /// <returns></returns>
    public static bool IsPlusEligible(LibraryType type)
    {
        return LibrariannPlusConfiguration.MetadataProvidersForLibraryTypes.ContainsKey(type);
    }
    ...
    public async Task FetchExternalDataTask(CancellationToken ct = default)
    {
        ...
        if (ids.Count == 0)
        {
            _logger.LogDebug("[Librariann+ Data Refresh] No series need matching or refreshing (stale data)");
            return;
        }


        _logger.LogDebug("[Librariann+ Data Refresh] Started Refreshing {Count} series data from Librariann+: {Ids}", ids.Count, string.Join(',', ids));
        var count = 0;
        ...
        _logger.LogDebug("[Librariann+ Data Refresh] Finished Refreshing {Count} / {Total} series data from Librariann+: {Ids}", count, ids.Count, string.Join(',', successfulMatches));
    }
    ...
    {
        // No Librariann+ license check here - series metadata matching is a core, free feature on this fork,
        // not gated behind a subscription that has no server behind it. See docs/... (n/a yet) - this was
        // silently no-op'ing metadata matching for every self-hosted install until removed.
        ...
        var result = await _librariannPlusApiService.MatchSeriesV3Async(matchRequest, ct);
        if (!result.IsSuccess)
        {
            _logger.LogError("Failed to load matches for series {SeriesId} from Librariann+: {Error}", seriesId, result.ErrorMessage);
            return null;
        }
        ...
        await _auditService.LogMatchAsync(LibrariannPlusEventType.SeriesMatched, seriesId,
    ...
    private async Task<ExternalSeriesMatchDto?> PickBestMatch(Series series, IList<ExternalSeriesMatchDto> matches, CancellationToken ct)
    {
        ...
        if (validAutomatedMatches.Count == 0)
        {
            ...
            await _auditService.LogAsync(LibrariannPlusAuditCategory.Match, LibrariannPlusEventType.SeriesBlacklisted,
    ...
    public async Task<IList<MalStackDto>> GetStacksForUser(int userId, CancellationToken ct = default)
    {
        ...
        try
        {
            _logger.LogDebug("Fetching Librariann+ for MAL Stacks for user {UserName}", scrobbleSettings.UserName);

            var license = (await _unitOfWork.SettingsRepository.GetSettingAsync(ServerSettingKey.LicenseKey, ct)).Value;
            return await _librariannPlusApiService.GetMalStacksAsync(scrobbleSettings.UserName, license, ct);
        }
        catch (Exception ex)
        {
            _logger.LogDebug(ex, "Fetching Librariann+ for MAL Stacks for user {UserName} failed", scrobbleSettings.UserName);
            return ArraySegment<MalStackDto>.Empty;
        }
    }

    /// <summary>
    /// Searches against Librariann+ for potential matched series/standalone books.
    /// </summary>
    ...
    public async Task<MatchSeriesResultDto?> MatchSeries(MatchSeriesDto dto, CancellationToken ct = default)
    {
        ...
        var kPlusResult = await _librariannPlusApiService.MatchSeriesV3Async(matchV3Request, ct);
    ...
    private sealed record QueriedExternalIds
    {
        ...
        /// <summary>
        /// If any id was found, the raw query string is meaningless to Librariann+ and shouldn't be sent along
        /// </summary>
    ...
    public async Task<ExternalSeriesDetailDto?> GetExternalSeriesDetail(int? aniListId, long? malId, int? mangaBakaId, int? seriesId, CancellationToken ct = default)
    {
        ...
        {
            throw new LibrariannException("Unable to find valid information from url for External Load");
        }
    ...
    {
        // See TryMatchAndLoadMetadataForSeries's comment - no license check on this fork.
        ...
        var data = await _unitOfWork.SeriesRepository.GetLibrariannPlusSeriesDetailRequestV3Dto(seriesId, ct);
        if (data == null) return _defaultReturn;

        // Get from Librariann+ API the Full Series metadata with rec/rev and cache to ExternalMetadata tables
        try
        {
    ...
    public async Task FixSeriesMatch(int seriesId, ExternalMetadataIdsDto ids, MetadataProvider? provider = null, CancellationToken ct = default)
    {
        ...
        _fileCacheService.InvalidatePrefix(GetCoversCacheKey(seriesId), FileCacheService.LibrariannPlusCacheDirectory);
        ...
        try
        {
            ...
            {
                _logger.LogError("Unable to Match {SeriesName} with Librariann+ Series with Ids: {AniListId}/{MalId}/{CbrId}/{MangabakaId}/{HardcoverId}",
            ...
            _logger.LogInformation("Matched {SeriesName} with Librariann+ Series {MatchSeriesName}", series.Name,
                metadata.Series.Name);
            await _auditService.LogMatchAsync(LibrariannPlusEventType.SeriesMatchFixed, seriesId,
        ...
        catch (LibrariannException ex)
        {
            ...
            _logger.LogInformation(ex, "Rate limit hit for matching {SeriesName} with Librariann+", series.Name);
    ...
    public async Task UpdateSeriesDontMatch(int seriesId, bool dontMatch, CancellationToken ct = default)
    {
        ...
        _logger.LogInformation("User has asked Librariann to stop matching/scrobbling on {SeriesName}", series.Name);
        ...
        await _auditService.LogMatchAsync(LibrariannPlusEventType.SeriesDontMatchSet, seriesId,
    ...
    public async Task UpdateSeriesMetadataProviderOverride(int seriesId, MetadataProvider? metadataProviderOverride, CancellationToken ct = default)
    {
        ...
        await _auditService.LogMatchAsync(LibrariannPlusEventType.SeriesMetadataProviderOverrideSet, seriesId,
        ...
        bool fromMatchFlow = false, MetadataFetchTrigger trigger = MetadataFetchTrigger.OnDemand, CancellationToken ct = default)
    {
        ...
        _logger.LogDebug("Fetching Librariann+ Series Detail data for {SeriesName}", string.IsNullOrEmpty(data.SeriesName) ? data.AniListId : data.SeriesName);

        await _auditService.LogAsync(
            LibrariannPlusAuditCategory.Metadata,
        ...
        var kPlusResult = await _librariannPlusApiService.GetSeriesDetailV3Async(data, ct);
        if (!kPlusResult.IsSuccess && (kPlusResult.ErrorMessage ?? string.Empty).Contains("Too many Requests"))
        {
            _logger.LogDebug("Hit the rate limit while fetching Librariann+ Series Detail data for {SeriesId}. Retrying in 3s", series.Id);
            await Task.Delay(3000, ct);

            kPlusResult = await _librariannPlusApiService.GetSeriesDetailV3Async(data, ct);
        }
        ...
        if (kPlusResult.ErrorMessage.IsUnknownSeriesError())
        {
            series.IsBlacklisted = true;
            await _unitOfWork.CommitAsync(ct);
            await _auditService.LogMatchAsync(LibrariannPlusEventType.SeriesBlacklisted, seriesId,
        ...
        if (result == null)
        {
            _logger.LogError("Unable to fetch Librariann+ Series Detail data for {SeriesId}: {ErrorMessage}",
            ...
            await _auditService.LogMatchAsync(LibrariannPlusEventType.SeriesMatchFailed, seriesId,
        ...
        if (result.Series != null && (series.Library!.AllowMetadataMatching || fromMatchFlow))
        {
            ...
            {
                _logger.LogError(ex, "There was an exception when trying to write Series metadata from Librariann+");
            }
        ...
        return new SeriesDetailPlusDto
        {
            ...
            Reviews = externalSeriesMetadata.ExternalReviews.Select(r => r.ToUserReviewDto()),
    ...
    /// <summary>
    /// Fetches volume and chapter covers from the Librariann+ covers endpoint and applies the best matches. Series and chapter
    ...
    {
        await _auditService.LogAsync(LibrariannPlusAuditCategory.Metadata, LibrariannPlusEventType.VolumeCoverUpdated, AuditStatus.Success,
    ...
    public async Task<IList<ExternalCoverResponseDto>> GetExternalCovers(int seriesId, int? volumeId = null, int? chapterId = null, CancellationToken ct = default)
    {
        ...
        if (series == null) throw new LibrariannException("Series not found");
        ...
        if (volumeId.HasValue)
        {
            ...
            if (volume == null) throw new LibrariannException("Volume not found");
        ...
        if (chapterId.HasValue)
        {
            ...
            if (chapter == null) throw new LibrariannException("Chapter not found");
        ...
        var result = await _fileCacheService.GetOrFetchAsync<KPlusResult<IList<ExternalCoverResponseDto>>>(
            cacheKey,
            FileCacheService.LibrariannPlusCacheDirectory,
            TimeSpan.FromDays(7),
            async _ => await _librariannPlusApiService.GetCoverImagesAsync(payload, ct),
    ...
    private async Task<List<SeriesStaffDto>> SetNameAndAddAliases(MetadataSettingsDto settings, IList<SeriesStaffDto>? staff)
    {
        ...
        foreach (var mapping in nameMappings)
        {
            ...
            if (existingPeopleDictionary.TryGetValue(mapping.AlternativeName.ToNormalized(), out var person))
            {
                ...
                await _auditService.LogPersonAsync(LibrariannPlusEventType.PersonAliasAdded, person.Id,
    ...
    private async Task<(bool, MetadataFieldChangeDto?)> UpdateCharacters(Series series, MetadataSettingsDto settings, IList<SeriesCharacter>? externalCharacters)
    {
        ...
        var characters = externalCharacters
            ...
            .Concat(series.Metadata.People
                ...
                .Where(p => !p.LibrariannPlusConnection)
                .Select(p => p.Person.ToPersonDto())
            )
        ...
        foreach (var spPerson in series.Metadata.People.Where(p => p.Role == PersonRole.Character))
        {
            ...
            {
                spPerson.LibrariannPlusConnection = true;
    ...
    private async Task<(bool, MetadataFieldChangeDto?)> UpdateArtists(Series series, MetadataSettingsDto settings, List<SeriesStaffDto> staff)
    {
        ...
        foreach (var person in series.Metadata.People.Where(p => p.Role == PersonRole.CoverArtist))
        {
            ...
            if (meta != null)
            {
                person.LibrariannPlusConnection = true;
            }
        }
    ...
    private (bool, MetadataFieldChangeDto?) UpdateAgeRating(Series series, MetadataSettingsDto settings, ExternalSeriesDetailDto externalMetadata, IEnumerable<string> allExternalTags)
    {
        ...
        try
        {
            ...
            // If the admin set up a raw mapping, then we use that, otherwise fallback to Librariann+'s mapping
    ...
    private static (bool, MetadataFieldChangeDto?) UpdateExternalIds(Series series, ExternalSeriesDetailDto externalMetadata)
    {
        ...
        // Add the rest of the Ids (Metron/ComicVine) when Librariann+ has them
    ...
    private async Task<bool> UpdateChapterCoverImage(Chapter chapter, MetadataSettingsDto settings, int seriesId, string? coverUrl)
    {
        ...
        {
            _logger.LogDebug("Librariann+ Update Chapter was skipped as cover was locked, Chapter: {ChapterId}", chapter.Id);
            return false;
        }

        await DownloadChapterCovers(chapter, coverUrl);
        chapter.AddKPlusOverride(MetadataSettingField.ChapterCovers);
        await _auditService.LogAsync(LibrariannPlusAuditCategory.Metadata, LibrariannPlusEventType.ChapterCoverUpdated, AuditStatus.Success,
    ...
    private async Task<bool> UpdateCoverImage(Series series, MetadataSettingsDto settings, ExternalSeriesDetailDto externalMetadata)
    {
        ...
        await _auditService.LogAsync(LibrariannPlusAuditCategory.Metadata, LibrariannPlusEventType.CoverUpdated, AuditStatus.Success,
    ...
    private async Task DownloadAndSetPersonCovers(List<SeriesStaffDto> people)
    {
        ...
        foreach (var staff in people)
        {
            ...
            try
            {
                ...
                await _auditService.LogPersonAsync(LibrariannPlusEventType.PersonCoverUpdated, person.Id,
    ...
    public static bool IsSeriesCompleted(Series series, List<Chapter> chapters, ExternalSeriesDetailDto externalMetadata, int maxVolumes)
    {
        ...
        // Note: If Librariann has specials, we should be lenient and ignore for the volume check
    ...
    /// <summary>
    /// This is to get series information for the recommendation drawer on Librariann
    /// </summary>
    ...
    private async Task<ExternalSeriesDetailDto?> GetSeriesDetail(int? aniListId, long? malId, int? mangaBakaId, int? seriesId, CancellationToken ct = default)
    {
        ...
        var result =  await _librariannPlusApiService.GetSeriesDetailV3Async(payload, ct);
        if (!result.IsSuccess)
        {
            _logger.LogError("Failed to retrieve series detail from Librariann Plus API: {ErrorMessage}", result.ErrorMessage);
            return null;
        }
