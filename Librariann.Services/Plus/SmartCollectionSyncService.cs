    ...
    : ISmartCollectionSyncService
{
    ...
    private async Task SyncCollection(AppUserCollection collection, CancellationToken ct = default)
    {
        ...
        if (info == null)
        {
            logger.LogInformation("Unable to find collection through Librariann+");
            await auditService.LogAsync(
                LibrariannPlusAuditCategory.Sync,
                LibrariannPlusEventType.SyncFailed,
                AuditStatus.Failure,
        ...
        try
        {
            ...
            await auditService.LogCollectionAsync(LibrariannPlusEventType.CollectionSynced, collection.Id,
    ...
    /// <summary>
    /// For each series in the upstream stack, find the matching Librariann series and link it to the collection if it
    ...
    internal async Task<CollectionLinkResult> LinkSeriesToCollection(AppUserCollection collection, SeriesCollection info, CancellationToken ct = default)
    {
        ...
        foreach (var seriesInfo in info.Series.OrderBy(s => s.SeriesName))
        {
            ...
            try
            {
                ...
                {
                    logger.LogDebug("Trying to find {SeriesName} ({Format}) within Librariann for linking. Found: {ExistingSeriesName} ({ExistingSeriesId})",
                    ...
                    await auditService.LogCollectionAsync(LibrariannPlusEventType.CollectionItemAdded, collection.Id,
    ...
    /// <summary>
    /// Resolve the Librariann series that corresponds to an upstream stack entry. The <paramref name="seriesInfo"/> already
    ...
    {
        logger.LogDebug("Fetching Librariann+ for MAL Stack");

        var license = (await unitOfWork.SettingsRepository.GetSettingAsync(ServerSettingKey.LicenseKey)).Value;

        var seriesForStack = await ($"{Configuration.LibrariannPlusApiUrl}/api/metadata/v2/stack?stackId=" + stackId)
