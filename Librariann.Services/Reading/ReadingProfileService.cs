...
namespace Librariann.Services.Reading;
...
public class ReadingProfileService(IUnitOfWork unitOfWork, ILocalizationService localizationService): IReadingProfileService
{
    ...
    public async Task<UserReadingProfileDto> GetReadingProfileDtoForSeries(int userId, int libraryId, int seriesId,
        int? activeDeviceId, bool skipImplicit = false)
    {
        return (await GetReadingProfileForSeries(userId, libraryId, seriesId,
            activeDeviceId, skipImplicit)).ToUserReadingProfileDto();
    }
        ...
        UserReadingProfileDto dto, int? activeDeviceId)
    {
        ...
        if (profile == null) throw new LibrariannException("profile-does-not-exist");
        ...
        return parentProfile.ToUserReadingProfileDto();
    }
    ...
    public async Task<UserReadingProfileDto> UpdateReadingProfile(int userId, UserReadingProfileDto dto)
    {
        ...
        await unitOfWork.CommitAsync();
        return profile.ToUserReadingProfileDto();
    }
    ...
    public async Task<UserReadingProfileDto> CreateReadingProfile(int userId, UserReadingProfileDto dto)
    {
        ...
        if (await unitOfWork.AppUserReadingProfileRepository.IsProfileNameInUse(userId, dto.Name)) throw new LibrariannException("name-already-in-use");
        ...
        return newProfile.ToUserReadingProfileDto();
    }
    ...
    public async Task<UserReadingProfileDto> PromoteImplicitProfile(int userId, int profileId, int? activeDeviceId)
    {
        ...
        if (profileToPromote == null) throw new LibrariannException("profile-does-not-exist");
        if (profileToPromote.Kind != ReadingProfileKind.Implicit) throw new LibrariannException("profile-not-implicit");
        ...
        if (series == null) throw new LibrariannException("series-doesnt-exist"); // Shouldn't happen
        ...
        return profileToPromote.ToUserReadingProfileDto();
    }
        ...
        UserReadingProfileDto dto, int? activeDeviceId)
    {
        ...
        if (existingProfile is {Kind: ReadingProfileKind.Implicit})
        {
            ...
            return existingProfile.ToUserReadingProfileDto();
        }
    ...
    public async Task DeleteReadingProfile(int userId, int profileId)
    {
        ...
        if (profile == null) throw new LibrariannException("profile-doesnt-exist");

        if (profile.Kind == ReadingProfileKind.Default) throw new LibrariannException("cant-delete-default-profile");
    ...
    public async Task SetSeriesProfiles(int userId, List<int> profileIds, int seriesId)
    {
        ...
        var selectedProfiles = profiles
            ...
            .ToList();
        if (selectedProfiles.Count != profileIds.Count) throw new LibrariannException("profile-doesnt-exist");
    ...
    public async Task SetProfileDevices(int userId, int profileId, List<int> deviceIds)
    {
        ...
        if (profile.Kind == ReadingProfileKind.Default) throw new LibrariannException("cant-assign-devices-to-default");
    ...
    private static void DeviceOverlapGuard(List<AppUserReadingProfile> profiles)
    {
        ...
        if (anyOverlap)
        {
            throw new LibrariannException("reading-profiles-device-overlap");
        }
    }
    ...
    public static void UpdateReaderProfileFields(AppUserReadingProfile existingProfile, UserReadingProfileDto dto, bool updateName = true)
    {
        ...
        existingProfile.BookReaderBackgroundColor = dto.BookReaderBackgroundColor;
        existingProfile.BookReaderBackgroundOpacity = dto.BookReaderBackgroundOpacity;
