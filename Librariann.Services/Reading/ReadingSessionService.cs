...
public sealed class ReadingSessionService : IReadingSessionService, IDisposable, IAsyncDisposable
{
    ...
    public async Task GenerateReadingSessionForChapters(int userId, int seriesId, Dictionary<int, int> chaptersMap, CancellationToken ct = default)
    {
        ...
        if (series == null) throw new LibrariannNotFoundException();
        ...
        ICacheService cacheService, IServiceScope scope)
    {
        ...
        // BookScrollId is an XPath for books read in the old (column-pagination) reader, but the foliate-js
        // reader stores an EPUB CFI there instead (see foliate-reader-poc.component.ts's saveProgress()) - CFI
        // syntax isn't valid XPath at all, so handing one to GetWordCountBetweenXPaths (which compiles it as an
        // XPath expression) throws on every single call for any book read in the new reader. There's no CFI-
        // based word-count equivalent implemented (yet) - skip the calculation entirely for CFI positions rather
        // than logging the same guaranteed exception on every progress save, same as the "no previous position"
        // branch below already does (WordsRead simply doesn't get a precise count for this activity).
        var startIsCfi = activity.StartBookScrollId?.StartsWith("epubcfi(", StringComparison.Ordinal) ?? false;
        var endIsCfi = progressDto.BookScrollId?.StartsWith("epubcfi(", StringComparison.Ordinal) ?? false;
