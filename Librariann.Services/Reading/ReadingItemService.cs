...
public class ReadingItemService : IReadingItemService
{
    ...
    private readonly AudiobookParser _audiobookParser;
        ...
        IAudiobookMetadataService audiobookMetadataService)
    {
        ...
        _audiobookParser = new AudiobookParser(directoryService);
    ...
    public int GetNumberOfPages(string filePath, MangaFormat format)
    {
        ...
        switch (format)
        {
            ...
            {
                // Audio has no "pages" - use a fixed 100-point scale so progress can be stored as a percentage
                // (PagesRead / Pages) and every existing page-ratio-based percent-complete/on-deck computation
                // works unmodified. The real resume position lives in AppUserProgress.PlaybackPositionSeconds.
                return 100;
            }
    ...
    public string GetCoverImage(string filePath, string fileName, MangaFormat format, EncodeFormat encodeFormat, CoverImageSize size = CoverImageSize.Default)
    {
        ...
        return format switch
        {
            ...
            MangaFormat.Audio => _audiobookMetadataService
                .GetCoverImageAsync(filePath, fileName, _directoryService.CoverImageDirectory, encodeFormat, size)
    ...
    public void Extract(string fileFilePath, string targetDirectory, MangaFormat format, int imageCount = 1)
    {
        switch (format)
        {
            ...
            case MangaFormat.Epub:
            case MangaFormat.Audio:
                // Audio files are streamed directly from their source path - never "extracted" into a cache dir.
                break;
            default:
    ...
    private ParserInfo? Parse(string path, string rootPath, string libraryRoot, LibraryType type, bool enableMetadata)
    {
        ...
        if (_audiobookParser.IsApplicable(path, type))
        {
            return _audiobookParser.Parse(path, rootPath, libraryRoot, type, enableMetadata, GetComicInfo(path, enableMetadata));
        }
