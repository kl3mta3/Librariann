    ...
    : IArchiveService
{
    ...
    public string CreateZipForDownload(IEnumerable<string> files, string tempFolder)
    {
        ...
        var potentialExistingFile = directoryService.FileSystem.FileInfo.New(Path.Join(directoryService.TempDirectory, $"librariann_{tempFolder}_{dateString}.zip"));
        ...
        {
            throw new LibrariannException("bad-copy-files-for-download");
        }

        var zipPath = Path.Join(directoryService.TempDirectory, $"librariann_{tempFolder}_{dateString}.zip");
        try
        {
        ...
        catch (AggregateException ex)
        {
            ...
            throw new LibrariannException("generic-create-temp-archive");
        }
    ...
    public string CreateZipFromFoldersForDownload(IList<string> files, string tempFolder, Func<Tuple<string, float>, Task> progressCallback)
    {
        ...
        var potentialExistingFile = directoryService.FileSystem.FileInfo.New(Path.Join(directoryService.TempDirectory, $"librariann_{tempFolder}_{dateString}.cbz"));
        if (potentialExistingFile.Exists)
        {
            // A previous download exists, just return it immediately
            return potentialExistingFile.FullName;
        ...
        var zipPath = Path.Join(directoryService.TempDirectory, $"librariann_{tempFolder}_{dateString}.cbz");
        try
        {
            ZipFile.CreateFromDirectory(tempLocation, zipPath);
            // Remove the folder as we have the zip
    ...
    public void ExtractArchive(string archivePath, string extractPath)
    {
        ...
        if (!directoryService.FileSystem.File.Exists(archivePath))
        {
            ...
            throw new LibrariannException($"{archivePath} does not exist on disk");
        }
