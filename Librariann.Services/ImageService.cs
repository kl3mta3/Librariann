    ...
    : IImageService
{
        ...
        int thumbnailWidth = ThumbnailWidth, int thumbnailHeight = ThumbnailHeight, string? targetDirectory = null)
    {
        ...
        {
            throw new LibrariannException("Invalid Base64 string", e);
        }
        catch (Exception e)
        {
    ...
    public async Task<string> CreateThumbnailFromUrl(string url, string fileName, EncodeFormat encodeFormat, int thumbnailWidth = ThumbnailWidth, int thumbnailHeight = ThumbnailHeight)
    {
        ...
        {
            // A 404 here isn't a real failure - providers like Open Library's covers.openlibrary.org (queried
            // with ?default=false) return one on purpose to mean "no image on file for this id" instead of a
            // generic placeholder. That's a routine, expected outcome (most people/series simply don't have a
            // cover there), not something worth alarming an admin over at Error level on every batch refresh.
            logger.LogDebug("No image available at {Url} (404)", url);
        }
        catch (Exception e)
        {
            logger.LogError(e, "Error creating thumbnail from url");
