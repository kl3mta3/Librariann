    ...
    : IEmailService
{
    ...
    public const string LibrariannPlusDebugTemplate = "LibrariannPlusDebug";
    public const string ServerNoticeTemplate = "ServerNotice";
    ...
    /// <summary>
    /// Sends a free-text notice from an admin to one recipient. Message is HTML-encoded first - it is admin-
    /// authored, but still untrusted input as far as the email template is concerned.
    /// </summary>
    public async Task SendServerNoticeEmail(int localeUserId, string emailAddress, string message)
    {
        var encodedMessage = System.Net.WebUtility.HtmlEncode(message).Replace("\n", "<br/>");

        var emailOptions = await CreateEmail()
            .ForTemplate(ServerNoticeTemplate)
            .WithLocalization(localeUserId, "server-notice")
            .WithPlaceholder("{{Message}}", encodedMessage)
            .To(emailAddress)
            .Build();
    ...
    /// <summary>
    /// Sends information about Librariann install for Librariann+ registration
    /// </summary>
    ...
    /// <returns></returns>
    public async Task<bool> SendLibrariannPlusDebug()
    {
        var settings = await unitOfWork.SettingsRepository.GetSettingsDtoAsync();
        if (!settings.IsEmailSetup()) return false;

        var emailOptions = await CreateEmail()
            .ForTemplate(LibrariannPlusDebugTemplate)
            .WithSubject("Librariann+: A User needs manual registration")
            .WithPreheader("Librariann+: A User needs manual registration")
