...
/// We can store the same one indefinitely as the authority does not change unless Librariann is restarted
/// </summary>
    ...
    [FromServices] ConfigurationManager<OpenIdConnectConfiguration>? configurationManager = null): IOidcService
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            throw new LibrariannException("errors.oidc.missing-external-id");
        }
        ...
        {
            throw new LibrariannException("errors.oidc.missing-email");
        }

        if (settings.RequireVerifiedEmail && !principal.HasVerifiedEmail())
        {
            throw new LibrariannException("errors.oidc.email-not-verified");
        }
        ...
        if (user != null)
        {
            ...
            {
                throw new LibrariannException("errors.oidc.email-in-use");
            }
    ...
    private async Task<AppUser?> CreateNewAccount(HttpRequest request, ClaimsPrincipal principal, OidcConfigDto settings, string oidcId)
    {
        ...
        if (settings.SyncUserSettings && !isAllowedToBeCreated)
        {
            ...
            throw new LibrariannException("errors.oidc.role-not-assigned");
        }
        ...
        catch (Exception e)
        {
            ...
            throw new LibrariannException("errors.oidc.creating-user");
        }
    ...
    private async Task<AppUser?> NewUserFromOpenIdConnect(HttpRequest request, OidcConfigDto settings, ClaimsPrincipal claimsPrincipal, string externalId)
    {
        ...
        var user = new AppUserBuilder(name, emailClaim.Value).Build();
    ...
    private async Task SetDefaults(OidcConfigDto settings, AppUser user)
    {
        ...
        if (errors.Any()) throw new LibrariannException("errors.oidc.syncing-user");
    ...
    private async Task SyncUserSettings(CookieValidatePrincipalContext ctx, OidcConfigDto settings, string idToken, AppUser user)
    {
        ...
        try
        {
            ...
            {
                throw new LibrariannException("errors.oidc.no-account");
            }
    ...
    public async Task SyncUserSettings(HttpRequest request, OidcConfigDto settings, ClaimsPrincipal claimsPrincipal, AppUser user)
    {
        ...
        catch (Exception ex)
        {
            ...
            throw new LibrariannException("errors.oidc.syncing-user", ex);
        }
    }
    ...
    private void SyncExtras(ClaimsPrincipal claimsPrincipal, AppUser user)
    {
        ...
        // will require action on Librariann. But it's less effort than saving if the user has set it themselves
    ...
    private async Task SyncEmail(HttpRequest request, OidcConfigDto settings, ClaimsPrincipal claimsPrincipal, AppUser user)
    {
        ...
        if (claimsPrincipal.HasVerifiedEmail())
        {
            ...
            if (!res.Succeeded)
            {
                ...
                throw new LibrariannException("errors.oidc.failed-to-update-email");
            }
        ...
        logger.LogInformation("Generated an OIDC email-change confirmation for user {UserId}", user.Id);
    ...
    private async Task SyncUsername(ClaimsPrincipal claimsPrincipal, AppUser user)
    {
        ...
        if (!res.Succeeded)
        {
            ...
            throw new LibrariannException("errors.oidc.failed-to-update-username");
        }
    }
    ...
    private async Task SyncRoles(OidcConfigDto settings, ClaimsPrincipal claimsPrincipal, AppUser user)
    {
        ...
        if (errors.Count != 0)
        {
            ...
            throw new LibrariannException("errors.oidc.syncing-user");
        }
    }
