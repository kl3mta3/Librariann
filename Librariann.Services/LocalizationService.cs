...
public class LocalizationService : ILocalizationService
{
    ...
    /// </summary>
    /// <returns></returns>
    public IEnumerable<LibrariannLocale> GetLocales()
    {
        if (_cache.TryGetValue(LocaleCacheKey, out List<LibrariannLocale>? cachedLocales) && cachedLocales != null)
        {
        ...
        var locales = new Dictionary<string, LibrariannLocale>();
        ...
        foreach (var file in uiLanguages)
        {
            ...
            {
                locales[fileName] = new LibrariannLocale
                {
                    FileName = fileName,
        ...
        var librariannLocales = locales.Values
            .Where(l => validFileNames.Contains(l.FileName))
            .ToList();
        _cache.Set(LocaleCacheKey, librariannLocales, _localsCacheOptions);

        return librariannLocales;
    }
