...
public class CollectionTagService(IUnitOfWork unitOfWork, IEventHub eventHub, IDirectoryService directoryService) : ICollectionTagService
{
    ...
    public async Task<bool> UpdateTag(AppUserCollectionDto dto, int userId, CancellationToken ct = default)
    {
        ...
        if (existingTag == null) throw new LibrariannException("collection-doesnt-exist");

        var user = await unitOfWork.UserRepository.GetUserByIdAsync(userId, ct: ct);
        if (user == null) throw new LibrariannException("user-doesnt-exist");

        var isAdmin = await unitOfWork.UserRepository.IsUserAdminAsync(user, ct);
        if (existingTag.AppUserId != userId && !isAdmin) throw new LibrariannException("access-denied");

        var title = dto.Title.Trim();
        if (string.IsNullOrEmpty(title)) throw new LibrariannException("collection-tag-title-required");
        ...
        if (!title.Equals(existingTag.Title) && await unitOfWork.CollectionTagRepository.CollectionExists(dto.Title, userId, ct))
            throw new LibrariannException("collection-tag-duplicate");

        existingTag.Items ??= [];
        if (existingTag.Source == ScrobbleProvider.Librariann)
        {
