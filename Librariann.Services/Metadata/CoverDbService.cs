...
using Librariann.Services.Repositories;
...
public class CoverDbService : ICoverDbService
{
    ...
    private const string NewHost = "https://www.librariannreader.com/CoversDB/";
    ...
    /// </returns>
    /// <exception cref="LibrariannException">
    ...
    public async Task<string> DownloadFaviconAsync(string url, EncodeFormat encodeFormat, CancellationToken ct = default)
    {
        ...
        if (res.HasValue)
        {
            ...
            _logger.LogTrace("Librariann has already tried to fetch from {BaseUrl} and failed. Skipping duplicate check", sanitizedBaseUrl);
            throw new LibrariannException($"Librariann has already tried to fetch from {sanitizedBaseUrl} and failed. Skipping duplicate check");
        }
        ...
        try
        {
            ...
            {
                correctSizeLink = await FallbackToLibrariannReaderFavicon(baseUrl);
            }
            if (string.IsNullOrEmpty(correctSizeLink))
            {
                throw new LibrariannException($"Could not grab favicon from {baseUrl}");
            }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            if (res.HasValue)
            {
                _logger.LogDebug("Librariann has already tried to fetch Publisher: {PublisherName} and failed. Skipping duplicate check", publisherName);
            ...
            var publisherLink = await FallbackToLibrariannReaderPublisher(publisherName);
            if (string.IsNullOrEmpty(publisherLink))
            {
                throw new LibrariannException($"Could not grab publisher image for {publisherName}");
            }
        ...
        CancellationToken ct = default)
    {
        try
        {
            ...
            {
                throw new LibrariannException($"Could not grab person image for {person.Name}");
            }
    ...
    private async Task<string?> GetCoverPersonImagePath(Person person)
    {
        ...
        {
            _logger.LogDebug("Inherited Librariann CoverDB lookup is disabled by Librariann configuration");
            return null;
        }
    ...
    private async Task<string> FallbackToLibrariannReaderFavicon(string baseUrl)
    {
        if (!Configuration.PremiumHostedServicesEnabled) return string.Empty;
        ...
        {
            throw new LibrariannException($"Could not grab favicon from {baseUrl.Sanitize()}");
        }

        return $"{NewHost}favicons/{externalFile}";
    }

    private async Task<string> FallbackToLibrariannReaderPublisher(string publisherName)
    {
        // Same guard as GetCoverPersonImagePath/FallbackToLibrariannReaderFavicon above - without it, this
        // unconditionally tried to reach NewHost (a domain this fork doesn't control/host) on every cache
        // miss, always failed, and got silently swallowed by the caller's catch block as a logged error.
        if (!Configuration.PremiumHostedServicesEnabled) return string.Empty;
