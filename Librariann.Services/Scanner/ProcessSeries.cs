...
using Librariann.Services.Builders;
    ...
    : IProcessSeries
{
    ...
    private async Task ReportDuplicateSeriesLookup(Library library, ParserInfo firstInfo, Exception ex)
    {
        ...
        var explanation =
            ...
            "Librariann cannot tell which one to update, so this folder was skipped.</p>";
    ...
    /// the normalized value that collided, so the user can see why Librariann treats the entries as the same series.
    /// </summary>
    ...
    /// <summary>
    /// Ensure that we don't overwrite Person metadata when all metadata is coming from Librariann+ metadata match functionality
    /// </summary>
    ...
    private static bool ShouldUpdatePeopleForRole(Series series, List<ChapterPeople> chapterPeople, PersonRole role)
    {
        ...
        // If metadata already has this role, but all entries are from LibrariannPlus, we should retain them
        if (series.Metadata.AnyOfRole(role))
        {
            var existingPeople = series.Metadata.People.Where(p => p.Role == role);

            // If all existing people are LibrariannPlus but new chapter people exist, we should still update
            if (existingPeople.All(p => p.LibrariannPlusConnection))
            {
                return false; // Ensure we don't remove LibrariannPlus people
            }
    ...
    private async Task UpdateCollectionTags(Series series, Chapter firstChapter)
    {
        ...
        logger.LogTrace("[TIME] Librariann took {Time} ms to process collections on Series: {Name}", sw.ElapsedMilliseconds, series.Name);
    }
    ...
    private async Task UpdateVolumes(Dictionary<string, Person> databasePeople, MetadataSettingsDto settings, Series series, IList<ParserInfo> parsedInfos, bool forceUpdate = false)
    {
        ...
        foreach (var volumeNumber in distinctVolumes)
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.LogCritical(ex, "[ScannerService] Librariann found corrupted volume entries on {SeriesName}. Please delete the series from Librariann via UI and rescan", series.Name);
                throw new LibrariannException(
                    $"Librariann found corrupted volume entries on {series.Name}. Please delete the series from Librariann via UI and rescan");
            }
            if (volume == null)
            {
    ...
    private async Task UpdateChapters(UpdateChapterArgs args)
    {
        ...
        foreach (var info in args.ParsedInfos)
        {
            ...
            // Add files
            await AddOrUpdateFileForChapter(chapter, info, args.ForceUpdate);
    ...
    private async Task AddOrUpdateFileForChapter(Chapter chapter, ParserInfo info, bool forceUpdate = false)
    {
        chapter.Files ??= [];
        ...
        if (existingFile != null)
        {
            ...
            if (info.Format == MangaFormat.Audio)
            {
                await ApplyAudioMetadata(existingFile, info.FullFilePath);
            }
        ...
        else
        {
            ...
            if (info.Format == MangaFormat.Audio)
            {
                await ApplyAudioMetadata(file, info.FullFilePath);
            }
    ...
    /// <summary>
    /// Reads duration/bitrate/embedded chapter markers via ffprobe (metadata-only, no transcoding/output) and
    /// stamps them onto the MangaFile. Runs at scan time so the audiobook reader has the data it needs without
    /// probing on every playback request.
    /// </summary>
    private async Task ApplyAudioMetadata(MangaFile file, string filePath)
    {
        var probe = await audiobookMetadataService.ProbeAsync(filePath);
        file.DurationSeconds = probe.DurationSeconds;
        file.Bitrate = probe.Bitrate;
        file.ChapterMarkersJson = probe.ChapterMarkers.Count > 0
            ? JsonSerializer.Serialize(probe.ChapterMarkers)
            : null;
    }
    ...
    private async Task UpdateChapterFromComicInfo(UpdateChapterComicInfoArgs args)
    {
        ...
        logger.LogTrace("[TIME] Librariann took {Time} ms to create/update Chapter: {File}", sw.ElapsedMilliseconds, chapter.Files.First().FileName);
    }
