...
public class TaskScheduler : ITaskScheduler
{
    ...
    public const string LibrariannPlusDataRefreshId = TaskSchedulerConstants.LibrariannPlusDataRefreshId;
    public const string LibrariannPlusStackSyncId = TaskSchedulerConstants.LibrariannPlusStackSyncId;
    public const string LibrariannPlusWantToReadSyncId = TaskSchedulerConstants.LibrariannPlusWantToReadSyncId;
    ...
    public const string PurgeLibrariannPlusAuditLogsId = TaskSchedulerConstants.PurgeLibrariannPlusAuditLogsId;
    public const string AcquisitionDownloadPollId = TaskSchedulerConstants.AcquisitionDownloadPollId;
    public const string AcquisitionAutomaticImportId = TaskSchedulerConstants.AcquisitionAutomaticImportId;
    public const string AcquisitionImportReconciliationId = TaskSchedulerConstants.AcquisitionImportReconciliationId;
    public const string MonitoringSearchId = TaskSchedulerConstants.MonitoringSearchId;
    public const string MonitoringCatalogSyncId = TaskSchedulerConstants.MonitoringCatalogSyncId;
    ...
    public TaskScheduler(ICacheService cacheService, ILogger<TaskScheduler> logger, IScannerService scannerService,
        ...
        ICleanupService cleanupService, IVersionUpdaterService versionUpdaterService,
    ...
    public async Task ScheduleTasks(CancellationToken cancellationToken = default)
    {
        ...
        RecurringJob.RemoveIfExists(SyncThemesTaskId);
        ...
        RecurringJob.AddOrUpdate<IAcquisitionQueueService>(AcquisitionDownloadPollId,
            service => service.PollAsync(CancellationToken.None),
            "* * * * *", RecurringJobOptions);
        RecurringJob.AddOrUpdate<IAcquisitionImportService>(AcquisitionAutomaticImportId,
            service => service.ProcessPendingAutomaticImportsAsync(CancellationToken.None),
            "* * * * *", RecurringJobOptions);
        RecurringJob.AddOrUpdate<IAcquisitionImportService>(AcquisitionImportReconciliationId,
            service => service.ReconcileImportedSeriesAsync(CancellationToken.None),
            "*/2 * * * *", RecurringJobOptions);
        RecurringJob.AddOrUpdate<IMonitoringJobService>(MonitoringSearchId,
            service => service.SearchDueAsync(CancellationToken.None),
            "*/15 * * * *", RecurringJobOptions);
        RecurringJob.AddOrUpdate<IMonitoringCatalogService>(MonitoringCatalogSyncId,
            service => service.SyncAllAsync(CancellationToken.None),
            "17 */6 * * *", RecurringJobOptions);
    ...
    public async Task ScheduleLibrariannPlusTasks(CancellationToken cancellationToken = default)
    {
        if (!Configuration.PremiumHostedServicesEnabled)
        {
            _logger.LogInformation("Inherited Librariann+ jobs are disabled by Librariann configuration");
            RemoveLibrariannPlusTasks();
            return;
        }
        ...
        // LibrariannPlus Scrobbling (every hour) - randomise minutes to spread requests out for K+
        var randomMinute = Rnd.Next(0, 60);
        _logger.LogDebug("Scheduling LibrariannPlus Scrobbling Task every hour @ {Minute}m", randomMinute);
        ...
        RecurringJob.AddOrUpdate(LibrariannPlusDataRefreshId,
    ...
    /// <summary>
    /// Removes any Librariann+ Recurring Jobs
    /// </summary>
    public static void RemoveLibrariannPlusTasks()
    {
        ...
        RecurringJob.RemoveIfExists(LibrariannPlusDataRefreshId);
        RecurringJob.RemoveIfExists(LibrariannPlusStackSyncId);
        RecurringJob.RemoveIfExists(LibrariannPlusWantToReadSyncId);
        RecurringJob.RemoveIfExists(PurgeLibrariannPlusAuditLogsId);
        RecurringJob.RemoveIfExists(TaskSchedulerConstants.CreateReadStatusTransitionRuleEventsId);
        RecurringJob.RemoveIfExists(TaskSchedulerConstants.RefreshConnectedTokensId);
    }
    ...
    public void ScheduleUpdaterTasks()
    {
        if (!Configuration.PremiumHostedServicesEnabled)
        {
            _logger.LogInformation("Inherited Librariann update checks are disabled by Librariann configuration");
            RecurringJob.RemoveIfExists(CheckForUpdateId);
            return;
        }
    ...
    /// <summary>
    /// Scans imported media and only queues its metadata refresh after the scanner has made the file authoritative.
    /// Imports into an existing series use the smaller series scan; imports that may create a series use a library
    /// scan and the reconciliation job resolves the new series before metadata work begins.
    /// </summary>
    public async Task ScanImportedMedia(int libraryId, int? seriesId, int acquisitionDownloadId)
    {
        if (HasScanTaskRunningForLibrary(libraryId) || RunningAnyTasksByMethod(ScanTasks, ScanQueue))
        {
            _logger.LogInformation(
                "A scan is already running; rescheduling post-import scan for acquisition download {DownloadId}",
                acquisitionDownloadId);
            BackgroundJob.Schedule(() => ScanImportedMedia(libraryId, seriesId, acquisitionDownloadId),
        ...
        {
            _logger.LogInformation("Enqueuing post-import series scan for acquisition download {DownloadId}",
                acquisitionDownloadId);
            jobId = BackgroundJob.Enqueue(() => _scannerService.ScanSeries(seriesId.Value, true));
            BackgroundJob.ContinueJobWith<IAcquisitionImportService>(jobId,
                service => service.QueueImportedSeriesMetadataRefreshAsync(acquisitionDownloadId,
                    CancellationToken.None));
        }
        else
        {
            _logger.LogInformation("Enqueuing post-import library scan for acquisition download {DownloadId}",
                acquisitionDownloadId);
            jobId = BackgroundJob.Enqueue(() => _scannerService.ScanLibrary(libraryId, false, true));
            BackgroundJob.ContinueJobWith<IAcquisitionImportService>(jobId,
                service => service.ReconcileImportedSeriesAsync(CancellationToken.None));
        }
