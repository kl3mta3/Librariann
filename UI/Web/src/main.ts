...
import {LibrariannTitleStrategy} from "./app/_services/librariann-title.strategy";
...
// All Languages Librariann will support: http://www.lingoes.net/en/translator/langcode.htm
const languageCodes = [
...
bootstrapApplication(AppComponent, {
    providers: [
        ...
        Title,
        { provide: TitleStrategy, useClass: LibrariannTitleStrategy },
...
.then(() => {
  if (environment.production && 'serviceWorker' in navigator) {
    window.addEventListener('load', () => {
      ...
      navigator.serviceWorker.register(workerUrl.pathname, {scope: new URL('.', document.baseURI).pathname})
        .catch(error => console.warn('Librariann service worker registration failed', error));
    });
  }
})
