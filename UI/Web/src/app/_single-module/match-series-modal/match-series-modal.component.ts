...
import {AllMetadataProviders, MetadataProvider} from "../../_models/librariannplus/metadata-provider.enum";
...
export class MatchSeriesModalComponent implements OnInit {
  ...
  librariannVolumeCount!: Signal<number>;
  librariannChapterCount!: Signal<number>;
  librariannSpecialCount!: Signal<number>;
  ...
  constructor() {
    ...
    this.librariannVolumeCount = computed(() => (this.seriesDetail()?.volumes ?? []).length);
    this.librariannChapterCount = computed(() => (this.seriesDetail()?.chapters ?? []).length);
    this.librariannSpecialCount = computed(() => (this.seriesDetail()?.specials ?? []).length);
