...
export class CoverImageChooserComponent  {
  ...
  protected readonly librariannplusImages = signal<CoverImageOption[]>([]);
  protected readonly librariannplusImagesLoaded = signal(false);
  ...
  private librariannPlusLoaded = false;
  ...
  selectOption(option: CoverImageOption, sourceTab: Tabs) {
    ...
    // Remote option (Volumes/Chapters/Librariann+/Other): stage it into temp, then emit the resulting filename.
    apiCall.subscribe({
      next: filename => {
  ...
  onTabActivate(tabId: Tabs) {
    ...
    } else if (tabId === Tabs.LibrariannPlus && !this.librariannPlusLoaded && this.config().librariannplusFunc) {
      this.librariannPlusLoaded = true;
      this.config().librariannplusFunc!.subscribe(items => {
        this.librariannplusImages.set(items);
        this.librariannplusImagesLoaded.set(true);
      });
