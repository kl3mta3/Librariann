...
export class BrowsePeopleComponent {
  ...
  refreshAuthorMetadata() {
    if (this.refreshingMetadata || !this.accountService.hasAdminRole()) return;
    this.refreshingMetadata = true;
    this.cdRef.markForCheck();
    this.personService.refreshAllAuthorMetadata().pipe(
      finalize(() => {
        this.refreshingMetadata = false;
        this.cdRef.markForCheck();
      })
    ).subscribe(() => this.toastr.success(translate('browse-people.metadata-refresh-queued')));
  }
}
