...
export class ReaderService {
  ...
  getNavigationArray(libraryId: number, seriesId: number, chapterId: number, format: MangaFormat) {
    ...
    if (format === MangaFormat.EPUB) {
      ...
      return ['library', libraryId, 'series', seriesId, 'audiobook', chapterId];
    } else {
  ...
  saveProgress(libraryId: number, seriesId: number, volumeId: number, chapterId: number, page: number,
               bookScrollId: string | null = null, playbackPositionSeconds: number | null = null) {
    return this.httpClient.post(this.baseUrl + 'reader/progress',
      {libraryId, seriesId, volumeId, chapterId, pageNum: page, bookScrollId, playbackPositionSeconds});
  }

  /**
   * Direct, range-request-seekable URL to an audiobook chapter's original audio file. No transcoding - the
   * browser's native <audio> element handles seeking via HTTP range requests.
   */
  getAudiobookStreamUrl(chapterId: number) {
    return `${this.baseUrl}reader/audiobook-stream?chapterId=${chapterId}&apiKey=${this.encodedKey}`;
  }

  /**
   * Direct URL to a book chapter's original EPUB file, for the foliate-js reader to fetch as a Blob and open
   * client-side (raw file, not the server-parsed/annotation-injected per-spine-item HTML from BookService).
   */
  getBookFileUrl(chapterId: number) {
    return `${this.baseUrl}reader/book-file?chapterId=${chapterId}&apiKey=${this.encodedKey}`;
  }

  getAudiobookChapterMarkers(chapterId: number) {
    return this.httpClient.get<Array<AudiobookChapterMarker>>(this.baseUrl + 'reader/audiobook-chapter-markers?chapterId=' + chapterId);
  }
  ...
  /**
   * Removes the Librariann UI aspect of the xpath from a given xpath variable
  ...
  /**
   * Adds the Librariann UI aspect to the xpath so loading from xpath in the reader works
   * @param xpath
   */
