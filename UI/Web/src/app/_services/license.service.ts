...
import {LibrariannPlusBillingInterval, LicenseInfo} from "../_models/librariannplus/license-info";
import {LibrariannPlusProductInfo} from "../_models/librariannplus/librariann-plus-product-info";
import {LibrariannPlusRegisterResult} from "../_models/librariannplus/registration/librariann-plus-register-result";
import {LibrariannPlusProviderHealthSnapshot} from '../_models/librariannplus/librariann-plus-provider-health';
...
export class LicenseService {
  ...
  /** Break the registration between Librariann+ and this instance */
  ...
  /** Has any license registered with the instance. Does not validate against Librariann+ API */
  hasAnyLicense() {
  ...
  updateUserLicense(license: string, email: string, discordId?: string) {
    return this.httpClient.post<LibrariannPlusRegisterResult>(this.baseUrl + 'license', {license, email, discordId}).pipe(
      tap(result => {
  ...
  getProviderHealthSnapshot(forceCheck = false) {
    return this.httpClient.get<LibrariannPlusProviderHealthSnapshot[]>(this.baseUrl + `license/provider-health?forceCheck=${forceCheck}`).pipe(
      map(res => res.filter(s => s.provider !== (3 as ScrobbleProvider)))); // Take out GoogleBooks, it's being repaced by Hardcover
  }

  getLicenseUsage() {
    return this.httpClient.get<LibrariannPlusLicenseUsage>(this.baseUrl + `license/stats`);
  }
  ...
  getProducts() {
    return this.httpClient.get<LibrariannPlusProductInfo[]>(this.baseUrl + 'license/products');
  }

  renewLicense(email: string, billingInterval: LibrariannPlusBillingInterval) {
