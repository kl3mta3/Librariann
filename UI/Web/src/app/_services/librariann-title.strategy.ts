...
export class LibrariannTitleStrategy extends TitleStrategy {
  ...
  override updateTitle(routerState: RouterStateSnapshot): void {
    ...
    // 3. Fallback
    this.title.setTitle('Librariann');
  }
  ...
  setFormattedTitle(pageTitle: string): void {
    ...
    this.title.setTitle(`${pageTitle} (Librariann)`);
  }

  setTranslatedTitle(key: string, params: Record<string, unknown>): void {
    this.title.setTitle(`${this.translocoService.translate(key, params)} (Librariann)`);
  }
