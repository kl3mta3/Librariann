...
export class NavService {
  ...
  public localStorageSideNavKey = 'librariann--sidenav--expanded';
  public localStorageSideNavGroupKeyPrefix = 'librariann--sidenav--group-expanded--';

  public navItems: NavItem[] = [
    // Smart Filters, Browse Genres, Browse Tags and Browse Annotations moved into the sidenav's
    // Browse group - they're always-visible destinations, not account-menu actions.
    {
  ...
  /**
   * Whether a collapsible sidenav group (Libraries, Browse, Smart Filters, etc) should be expanded.
   * Falls back to defaultValue when the user has never toggled that group before.
   */
  isSideNavGroupExpanded(groupId: string, defaultValue: boolean): boolean {
    const raw = localStorage.getItem(this.localStorageSideNavGroupKeyPrefix + groupId);
    return raw === null ? defaultValue : raw === 'true';
  }

  setSideNavGroupExpanded(groupId: string, expanded: boolean): void {
    localStorage.setItem(this.localStorageSideNavGroupKeyPrefix + groupId, expanded + '');
  }
}
