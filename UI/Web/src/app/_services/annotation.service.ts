...
/** Same formatting `SlotColorPipe` uses - duplicated as a plain function since this runs outside a template. */
function rgbaCssValue(color: RgbaColor): string {
...
export class AnnotationService {
  ...
  /**
   * Prefix for the CSS custom properties (`--librariann-highlight-slot-0` etc.) kept in sync with the user's
   * highlight slot colors below - readable by ANY renderer (the current one already computes an inline
   * rgba() per element instead; the foliate-js reader's annotation overlay uses these directly as SVG fill
   * values) without each needing its own copy of the color logic. A slot-color change here propagates to every
   * already-drawn overlay highlight instantly, with no re-render call needed - the same var()-driven approach
   * the eventual site-wide theme system (accent colors, surfaces, etc.) should use too.
   */
  static readonly HIGHLIGHT_SLOT_CSS_VAR_PREFIX = '--librariann-highlight-slot-';
    ...
    effect(() => {
      for (const slot of this.slots()) {
        this.document.documentElement.style.setProperty(
          `${AnnotationService.HIGHLIGHT_SLOT_CSS_VAR_PREFIX}${slot.slotNumber}`,
          rgbaCssValue(slot.color),
        );
      }
    });
  }
  ...
  /**
   * One-time positional backfill for a pre-foliate-js annotation - see Annotation.cfi and the migration plan.
   * Separate from any general "edit my annotation" call since this isn't user-editable content.
   */
  setAnnotationCfi(annotationId: number, cfi: string) {
    return this.httpClient.post(this.baseUrl + `annotation/${annotationId}/cfi`, {cfi});
  }
