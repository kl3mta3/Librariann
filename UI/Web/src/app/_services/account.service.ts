...
import {InviteRequest} from "../_models/auth/invite-request";
import {RequestInviteResponse} from "../_models/auth/request-invite-response";
...
export enum Role {
  ...
  GrabReleases = 'Grab Releases',
  ManageMetadata = 'Manage Metadata',
  ManageLibraries = 'Manage Libraries',
  ManageAcquisition = 'Manage Acquisition',
}
...
export const allRoles = [
  ...
  Role.GrabReleases,
  Role.ManageMetadata,
  Role.ManageLibraries,
  Role.ManageAcquisition,
]
...
export class AccountService {
  ...
  public static userKey = 'librariann-user';
  public static lastLoginKey = 'librariann-lastlogin';
  public static localeKey = 'librariann-locale';
  ...
  public readonly canSearchIndexers = computed(() => this.hasRole(this._currentUser(), Role.SearchIndexers) || this.hasRole(this._currentUser(), Role.Admin));
  public readonly canGrabReleases = computed(() => this.hasRole(this._currentUser(), Role.GrabReleases) || this.hasRole(this._currentUser(), Role.Admin));
  public readonly canManageMetadata = computed(() => this.hasRole(this._currentUser(), Role.ManageMetadata) || this.hasRole(this._currentUser(), Role.Admin));
  public readonly canManageLibraries = computed(() => this.hasRole(this._currentUser(), Role.ManageLibraries) || this.hasRole(this._currentUser(), Role.Admin));
  public readonly canManageAcquisition = computed(() => this.hasRole(this._currentUser(), Role.ManageAcquisition) || this.hasRole(this._currentUser(), Role.Admin));
  ...
  requestInvite(model: {email: string, name: string}) {
    return this.httpClient.post<RequestInviteResponse>(this.baseUrl + 'account/request-invite', model);
  }

  /** The login screen's "Request an Invite" link visibility. Anonymous-safe. */
  isInviteRequestEnabled() {
    return this.httpClient.get<boolean>(this.baseUrl + 'account/invite-request-enabled');
  }

  getPendingInvites() {
    return this.httpClient.get<Array<InviteRequest>>(this.baseUrl + 'account/pending-invites');
  }

  approveInviteRequest(id: number) {
    return this.httpClient.post<InviteUserResponse>(this.baseUrl + 'account/approve-invite/' + id, {});
  }

  inviteAllPending() {
    return this.httpClient.post<number>(this.baseUrl + 'account/invite-all-pending', {});
  }

  getInviteSettings() {
    return this.httpClient.get<UserInviteSettings>(this.baseUrl + 'account/invite-settings');
  }

  updateInviteSettings(model: UserInviteSettings) {
    return this.httpClient.post<UserInviteSettings>(this.baseUrl + 'account/invite-settings', model);
  }

  notifyUsers(model: {adminsOnly: boolean, message: string}) {
    return this.httpClient.post<number>(this.baseUrl + 'account/notify-users', model);
  }
