...
export interface ReaderSettingUpdate {
  setting: 'pageStyle' | 'clickToPaginate' | 'fullscreen' | 'writingStyle' | 'layoutMode' | 'readingDirection' | 'immersiveMode' | 'theme' | 'appearance' | 'pageCalcMethod' | 'bookReaderDisableBookmarkIcon';
  object: any;
}
...
export type BookReadingProfileFormGroup = FormGroup<{
  ...
  bookReaderBackgroundColor: FormControl<string>;
  bookReaderBackgroundOpacity: FormControl<number>;
...
export class EpubReaderSettingsService {
  ...
  private readonly _backgroundColor = signal<string>('#F1E4D5');
  private readonly _backgroundOpacity = signal<number>(100);
  ...
  constructor() {
    ...
    effect(() => {
      ...
      this.settingUpdateSubject.next({setting: 'appearance', object: appearance});
    });
  }
  ...
  private setupDefaultsFromProfile(profile: ReadingProfile): void {
    ...
    if (!/^#[0-9A-Fa-f]{6}$/.test(profile.bookReaderBackgroundColor ?? '')) {
      profile.bookReaderBackgroundColor = '#F1E4D5';
    }
    if (profile.bookReaderBackgroundOpacity === undefined || profile.bookReaderBackgroundOpacity < 0 || profile.bookReaderBackgroundOpacity > 100) {
      profile.bookReaderBackgroundOpacity = 100;
    }
    ...
    this._backgroundColor.set(profile.bookReaderBackgroundColor);
    this._backgroundOpacity.set(profile.bookReaderBackgroundOpacity);
  ...
  private updateFormFromSignals(): void {
    ...
    this.settingsForm.patchValue({
      ...
      bookReaderBackgroundColor: this._backgroundColor(),
      bookReaderBackgroundOpacity: this._backgroundOpacity()
    }, { emitEvent: false });
  }
  ...
  private setupSettingsForm(): void {
    ...
    this.settingsForm = new FormGroup({
      ...
      bookReaderBackgroundColor: this.fb.control(profile.bookReaderBackgroundColor),
      bookReaderBackgroundOpacity: this.fb.control(profile.bookReaderBackgroundOpacity),
  ...
  private setupFormSubscriptions(): void {
    ...
    this.settingsForm.get('bookReaderBackgroundColor')?.valueChanges.pipe(
      takeUntilDestroyed(this.destroyRef)
    ).subscribe(color => this._backgroundColor.set(color));

    this.settingsForm.get('bookReaderBackgroundOpacity')?.valueChanges.pipe(
      takeUntilDestroyed(this.destroyRef)
    ).subscribe(opacity => this._backgroundOpacity.set(opacity));
  ...
  private packReadingProfile(): ReadingProfile {
    ...
    data.bookReaderBackgroundColor = this._backgroundColor();
    data.bookReaderBackgroundOpacity = this._backgroundOpacity();
