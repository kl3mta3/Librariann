...
export class LibrarySettingsModalComponent implements OnInit {
  ...
  protected readonly excludePatternTooltip = translate('library-settings-modal.exclude-patterns-tooltip');
  ...
  constructor() {
    effect(() => {
      ...
      // Some library types (e.g. Audiobook) have no metadata provider at all - leave the field alone rather
      // than forcing it to undefined, since there's nothing valid to fall back to.
      if (validMetadataProviders.length > 0 && !validMetadataProviders.includes(selectedMetadataProvider)) {
