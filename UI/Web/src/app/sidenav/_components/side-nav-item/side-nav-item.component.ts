...
import {APP_BASE_HREF, NgClass, NgTemplateOutlet} from "@angular/common";
...
export class SideNavItemComponent {
  ...
  externalSourceId = input<number>(0);
  /**
  ...
  openExternal(event: MouseEvent) {
    const externalSourceId = this.externalSourceId();
    if (externalSourceId <= 0) return;

    event.preventDefault();
    const popup = window.open('about:blank', '_blank');
    if (popup) popup.opener = null;
    this.externalSourceService.createLaunch(externalSourceId).subscribe({
      next: path => {
