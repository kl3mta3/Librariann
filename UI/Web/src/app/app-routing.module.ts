...
import {indexerSearchGuard} from './_guards/indexer-search.guard';
import {manageAcquisitionGuard} from './_guards/manage-acquisition.guard';
import {manageMetadataGuard} from './_guards/manage-metadata.guard';
...
export const routes: Routes = [
  {
    ...
    children: [
      ...
      },
      {
        path: 'embed',
        title: 'title.home',
        loadComponent: () => import('./dashboard/_components/dashboard.component').then(c => c.DashboardComponent)
      },
      {
        path: 'discover',
        title: 'Discover',
        loadComponent: () => import('./discover/discover.component').then(c => c.DiscoverComponent)
      },
      {
        path: 'release-search',
        canActivate: [indexerSearchGuard],
        title: 'title.release-search',
        loadComponent: () => import('./acquisition/interactive-search/interactive-search.component').then(c => c.InteractiveSearchComponent)
      },
      {
        path: 'acquisition-activity',
        canActivate: [manageAcquisitionGuard],
        title: 'title.acquisition-activity',
        loadComponent: () => import('./acquisition/acquisition-activity/acquisition-activity.component').then(c => c.AcquisitionActivityComponent)
      },
      {
        path: 'monitoring',
        canActivate: [manageAcquisitionGuard],
        title: 'title.monitoring',
        loadComponent: () => import('./acquisition/monitoring/monitoring.component').then(c => c.MonitoringComponent)
      },
      {
        path: 'metadata-discovery',
        canActivate: [manageMetadataGuard],
        title: 'title.metadata-discovery',
        loadComponent: () => import('./metadata-discovery/metadata-discovery.component').then(c => c.MetadataDiscoveryComponent)
      },
      {
        path: 'person/:name',
        runGuardsAndResolvers: 'always',
      ...
      },
      {
        ...
        children: [
          ...
          {
            // foliate-js based reader - replaced the old column-pagination reader once it reached full
            // feature parity (pagination, progress, annotations, settings, TTS). See the migration plan.
            path: 'series/:seriesId/book',
            loadChildren: () => import('./_routes/book-reader-v2.router.module').then(m => m.routes)
          },
          {
          ...
          {
            path: 'series/:seriesId/audiobook',
