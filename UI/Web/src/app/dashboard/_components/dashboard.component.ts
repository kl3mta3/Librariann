...
export class DashboardComponent {
  ...
  favoriteGenreSeries$ = this.seriesService.getRecentlyAddedInFavoriteGenres(1, 20).pipe(
    map(page => page.result.map(series => CardEntityFactory.series(series))),
    shareReplay({bufferSize: 1, refCount: true}),
  );
  recentlyDownloadedSeries$ = this.seriesService.getRecentlyDownloaded(1, 20).pipe(
    map(page => page.result.map(series => CardEntityFactory.series(series))),
    shareReplay({bufferSize: 1, refCount: true}),
  );
  missingFromMonitoredSeries$ = this.seriesService.getMissingFromMonitoredSeries(1, 20).pipe(
    map(page => page.result),
    shareReplay({bufferSize: 1, refCount: true}),
  );
  becauseYouReadSeries$ = this.seriesService.getBecauseYouRead(1, 20).pipe(
    map(page => page.result.map(series => CardEntityFactory.series(series))),
    shareReplay({bufferSize: 1, refCount: true}),
  );
  newFromFollowedAuthors$ = this.seriesService.getNewFromFollowedAuthors(1, 20).pipe(
    map(page => page.result.map(series => CardEntityFactory.series(series))),
    shareReplay({bufferSize: 1, refCount: true}),
  );
  nextInSeries$ = this.seriesService.getNextInSeries(1, 20).pipe(
    map(page => page.result.map(series => CardEntityFactory.series(series))),
    shareReplay({bufferSize: 1, refCount: true}),
  );

  recentlyDownloadedNextPage = (pageNum: number, pageSize: number) => {
    return this.seriesService.getRecentlyDownloaded(pageNum, pageSize)
      .pipe(map(page => page.result.map(series => CardEntityFactory.series(series))));
  };

  becauseYouReadNextPage = (pageNum: number, pageSize: number) => {
    return this.seriesService.getBecauseYouRead(pageNum, pageSize)
      .pipe(map(page => page.result.map(series => CardEntityFactory.series(series))));
  };

  newFromFollowedAuthorsNextPage = (pageNum: number, pageSize: number) => {
    return this.seriesService.getNewFromFollowedAuthors(pageNum, pageSize)
      .pipe(map(page => page.result.map(series => CardEntityFactory.series(series))));
  };

  nextInSeriesNextPage = (pageNum: number, pageSize: number) => {
    return this.seriesService.getNextInSeries(pageNum, pageSize)
      .pipe(map(page => page.result.map(series => CardEntityFactory.series(series))));
  };

  missingSeriesCover(item: MissingSeriesItem): string {
    return this.imageService.getSeriesCoverImage(item.sourceSeriesId);
  }
  ...
  favoriteGenreNextPage = (pageNum: number, pageSize: number) => {
    return this.seriesService.getRecentlyAddedInFavoriteGenres(pageNum, pageSize)
      .pipe(map(page => page.result.map(series => CardEntityFactory.series(series))));
  };
