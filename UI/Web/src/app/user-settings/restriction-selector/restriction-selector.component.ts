...
export class RestrictionSelectorComponent {
  ...
  readonly audiencePresets = [
    {key: 'kid', icon: 'fa-child-reaching', ageRating: AgeRating.Everyone},
    {key: 'youth', icon: 'fa-graduation-cap', ageRating: AgeRating.Everyone10Plus},
    {key: 'teen', icon: 'fa-user', ageRating: AgeRating.Teen},
    {key: 'adult', icon: 'fa-user-shield', ageRating: AgeRating.NotApplicable},
  ];
  ...
  applyAudiencePreset(ageRating: AgeRating) {
    if (this.isAdmin()) return;
    this.restrictionForm.get('ageRating')!.setValue(ageRating);
  }

  isAudiencePresetSelected(ageRating: AgeRating): boolean {
    return Number(this.restrictionForm.get('ageRating')!.value) === ageRating;
  }
}
