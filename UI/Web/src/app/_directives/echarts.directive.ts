...
export class EChartsDirective implements OnInit, OnDestroy {
  ...
  constructor() {
    ...
    effect(() => {
      const themeName = this.themeService.themeName(); // Call on theme updates
      const options = untracked(this.options);
      if (!options) return;

      this.ngZone.runOutsideAngular(() => {
        registerTheme(themeName, this.createTheme());

        this.echart?.dispose();
        this.echart = init(this.el.nativeElement, themeName, untracked(this.initOptions));
  ...
  ngOnInit() {
    ...
    this.ngZone.runOutsideAngular(() => {
      ...
      this.echart = init(this.el.nativeElement, themeName, this.initOptions());
      this.echart.setOption(options);
    });
