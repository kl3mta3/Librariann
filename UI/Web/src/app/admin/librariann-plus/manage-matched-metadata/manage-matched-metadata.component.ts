...
import {allMatchStates, MatchStateOption} from "../../../_models/librariannplus/match-state-option";
...
export class ManageMatchedMetadataComponent implements OnInit {
  ...
  loadData(pageNumber: number = 1) {
    ...
    return this.manageService.getAllLibrariannPlusSeries(filter, pageNumber, this.pagination().itemsPerPage).pipe(tap(data => {
      this.data.set(data.result);
      this.pagination.set({
        itemsPerPage: data.pagination.itemsPerPage,
        totalItems: data.pagination.totalItems,
        totalPages: data.pagination.totalPages,
        currentPage: data.pagination.currentPage - 1, // ngx-datatable is 0 based, Librariann is 1 based
      });
