...
export class ManageLicenseModalComponent {
  ...
  protected readonly isActiveSubscription = computed((): boolean => {
    ...
    return state === LibrariannPlusSubscriptionState.Active
      || state === LibrariannPlusSubscriptionState.Cancelling
      || state === LibrariannPlusSubscriptionState.Paused;
  });

  protected readonly isCancelling = computed((): boolean =>
    this.licenseInfo()?.state === LibrariannPlusSubscriptionState.Cancelling);
