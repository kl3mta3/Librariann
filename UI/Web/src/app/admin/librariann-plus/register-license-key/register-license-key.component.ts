...
import {
  LibrariannPlusRegistrationErrorCode
} from '../../../_models/librariannplus/registration/librariann-plus-registration-error-code';
import {LibrariannPlusRegistrationErrorCodePipe} from '../../../_pipes/registration-error-code.pipe';
...
@Component({
  ...
  imports: [
    ...
    TranslocoDirective,
    LibrariannPlusRegistrationErrorCodePipe,
  ],
...
export class RegisterLicenseKeyComponent {
  ...
  protected readonly errorCode = signal<LibrariannPlusRegistrationErrorCode | null>(null);
  ...
  save() {
    ...
    this.licenseService.registerLicense(this.formData.licenseKey, this.formData.email, this.formData.discordId ?? undefined).subscribe(async result => {
      ...
      if (!result.success) {
        if (result.errorCode === LibrariannPlusRegistrationErrorCode.AlreadyRegistered) {
        ...
        this.errorCode.set(result.errorCode ?? LibrariannPlusRegistrationErrorCode.InternalError);
        return;
      }
