...
import {
  LibrariannPlusBillingInterval,
  LibrariannPlusSubscriptionState,
  LicenseInfo
} from '../../../_models/librariannplus/license-info';
...
@Component({
  ...
  imports: [UtcToLocalTimePipe, UpperCasePipe, TranslocoDirective, LibrariannPlusSubscriptionStatusPipe, LibrariannPlusBillingIntervalPipe],
...
export class LicenseInfoPanelComponent {
  ...
  readonly status = computed((): LibrariannPlusSubscriptionState => {
    return this.licenseInfo()?.state ?? LibrariannPlusSubscriptionState.Expired;
  });
  ...
  readonly daysUntilExpiryPercent = computed((): number => {
    ...
    const totalDays = interval === LibrariannPlusBillingInterval.Year ? 365
      : interval === LibrariannPlusBillingInterval.Week ? 7
      : interval === LibrariannPlusBillingInterval.Day  ? 1
      : 30;
  ...
  protected readonly LibrariannPlusSubscriptionState = LibrariannPlusSubscriptionState;
