...
export class InviteUserComponent implements OnInit {
  ...
  /**
   * When true, this is the "Set Default Permissions" flow from the Users settings tab rather than a real
   * invite - the email field is hidden, the footer button saves the roles/libraries/age-restriction as the
   * server's default invite permissions instead of inviting anyone.
   */
  isDefaultPermissionsMode = input(false);
  ...
  ngOnInit(): void {
    ...
    if (this.isDefaultPermissionsMode()) {
      this.email?.clearValidators();
      this.email?.updateValueAndValidity();

      this.accountService.getInviteSettings().subscribe(settings => {
        const permissions = settings.defaultInvitePermissions;
        this.inviteForm.patchValue({
          libraries: permissions.libraries,
          roles: permissions.roles as Role[],
        });
        this.selectedRestriction = permissions.ageRestriction;
  ...
  save() {
    this.isSaving = true;
    const {roles, libraries} = this.inviteForm.getRawValue();

    // Preserve the two invite toggles - this form only ever edits the default permission set.
    this.accountService.getInviteSettings().subscribe(current => {
      this.accountService.updateInviteSettings({
        ...current,
        defaultInvitePermissions: {roles, libraries, ageRestriction: this.selectedRestriction}
      }).subscribe({
        next: () => {
          this.isSaving = false;
