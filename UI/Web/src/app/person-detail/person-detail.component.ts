...
export class PersonDetailComponent {
  ...
  isAuthor = computed(() => this.person().roles?.includes(PersonRole.Writer) ?? false);
  personActions = this.actionService.getPersonActions();
  authorMatches = signal<AuthorMetadataCandidate[]>([]);
  matchingAuthors = signal(false);
  applyingAuthorId = signal<string | null>(null);
  showAuthorMatches = signal(false);
  ...
  toggleFollow() {
    ...
    const followed = !person.isFollowed;
    this.personService.setFollowed(person.id, followed).subscribe(value =>
      this.person.set({...person, isFollowed: value}));
  }

  findAuthorMatch() {
    this.showAuthorMatches.set(true);
    this.matchingAuthors.set(true);
    this.authorMatches.set([]);
    this.personService.searchAuthorMetadata(this.person().name).pipe(
      finalize(() => this.matchingAuthors.set(false))
    ).subscribe({
      next: matches => this.authorMatches.set(matches),
      error: () => this.toastr.error('Open Library author search failed. Please try again.')
    });
  }

  applyAuthorMatch(candidate: AuthorMetadataCandidate) {
    this.applyingAuthorId.set(candidate.externalId);
    this.personService.applyAuthorMetadata(this.person().id, candidate).pipe(
      finalize(() => this.applyingAuthorId.set(null))
    ).subscribe({
      next: person => {
        this.setPerson(person);
        this.showAuthorMatches.set(false);
        this.toastr.success(`Matched ${person.name} with Open Library`);
      },
      error: () => this.toastr.error('The author match could not be applied.')
    });
  }
