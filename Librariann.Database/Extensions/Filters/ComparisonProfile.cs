...
namespace Librariann.Database.Extensions.Filters;
...
public static class ComparisonProfile
{
    ...
    /// <summary>
    /// Throws <see cref="LibrariannException"/> if the comparison is not in the allowed set.
    /// </summary>
    ...
    public static void Validate(FilterComparison comparison, HashSet<FilterComparison> allowed, string fieldName)
    {
        if (!allowed.Contains(comparison))
            throw new LibrariannException($"{comparison} is not applicable for {fieldName}");
    }
}
