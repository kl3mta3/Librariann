...
public class ReadingListRepository(DataContext context) : IReadingListRepository
{
        ...
        UserParams userParams, bool sortByLastModified = true, CancellationToken ct = default)
    {
       ...
       var finalQuery = query.Select(ReadingListMapping.ToReadingListDtoExpression)
            .AsNoTracking();
        ...
        UserParams? userParams = null, CancellationToken ct = default)
    {
        ...
        var query = context.ReadingListItem
            ...
            .Select(ReadingListItemMapping.ToReadingListItemDtoExpression(userId))
            .AsSplitQuery();
    ...
    public async Task<List<ReadingListTagDto>> GetAllReadingListTagDtosAsync(int userId, CancellationToken ct = default)
    {
        return await context.ReadingList
            ...
            .SelectMany(r => r.Tags)
            .Distinct()
            .Select(ReadingListTagMapping.ToReadingListTagDtoExpression)
            .ToListAsync(ct);
    }
        ...
        AgeRestriction ageRating, CancellationToken ct = default)
    {
        ...
        return limitedQuery.Select(ReadingListMapping.ToReadingListDtoExpression);
    }
