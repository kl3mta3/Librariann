...
using Librariann.Database.Converters;
using Librariann.Database.Extensions;
using Librariann.Database.Extensions.Filters;
...
using Librariann.Models.DTOs.Filtering.v2.SortFields;
using Librariann.Models.DTOs.Filtering.v2.SortOptions;
...
public class SeriesRepository(DataContext context) : ISeriesRepository
{
...
#nullable  enable
        ...
        SearchDto dto, CancellationToken ct = default)
    {
        ...
        var librariesTask = dto.HasShortcode
            ...
            : context.Library
                ...
                .Select(LibraryMapping.ToLibraryDtoExpression)
                .ToListAsync(ct);
        ...
        var annotationsTask = dto.HasShortcode
            ...
            : context.AppUserAnnotation
                ...
                .Select(AnnotationMapping.ToAnnotationDtoExpression)
                .ToListAsync(ct);
        #endregion
        ...
        var seriesTask = baseSeriesQuery
            ...
            .Take(maxRecords)
            .Select(SearchResultMapping.ToSearchResultDtoExpression)
            .ToListAsync(ct);
        ...
        var readingListsTask = dto.HasShortcode
            ...
            : context.ReadingList
                ...
                .Take(maxRecords)
                .Select(ReadingListMapping.ToReadingListDtoExpression)
                .ToListAsync(ct);
        ...
        var collectionsTask = dto.HasShortcode
            ...
            : context.AppUserCollection
                ...
                .Select(AppUserCollectionMapping.ToAppUserCollectionDtoExpression)
                .ToListAsync(ct);
        ...
        var genresTask = dto.HasShortcode
            ...
            : context.Genre
                ...
                .Take(maxRecords)
                .Select(GenreTagMapping.ToGenreTagDtoExpression)
                .ToListAsync(ct);
        ...
        var tagsTask = dto.HasShortcode
            ...
            : context.Tag
                ...
                .Take(maxRecords)
                .Select(GenreTagMapping.ToTagDtoExpression)
                .ToListAsync(ct);
        ...
        if (dto.IncludeChapterAndFiles && (hasQuery || hardcoverId > 0))
        {
            ...
            result.Chapters = await chaptersQuery
                ...
                .Take(maxRecords)
                .Select(ChapterMapping.ToChapterDtoExpression(0)) // Matches the original unparameterized ProjectTo<ChapterDto>
                .ToListAsync(ct);
            ...
            if (isAdmin && hasQuery)
            {
                result.Files = await context.MangaFile
                    ...
                    .Take(maxRecords)
                    .Select(MangaFileMapping.ToMangaFileDtoExpression)
                    .ToListAsync(ct);
            }
        }
    ...
    public async Task<SeriesDto?> GetSeriesDtoByIdAsync(int seriesId, int userId, CancellationToken ct = default)
    {
        var series = await context.Series
            ...
            .Select(SeriesMapping.ToSeriesDtoExpression(userId))
            .SingleOrDefaultAsync(ct);
        ...
        CancellationToken ct = default)
    {
        ...
        return await context.Series
            ...
            .AsSplitQuery()
            .Select(SeriesMapping.ToSeriesDtoExpression(user.Id))
            .ToListAsync(ct);
    }
        ...
        CancellationToken ct = default)
    {
        return await context.SeriesMetadata
            ...
            .AsNoTracking()
            .Select(SeriesMetadataMapping.ToSeriesMetadataDtoExpression)
            .AsSplitQuery()
            .ToListAsync(ct);
    }
        ...
        SeriesFilterV2Dto seriesFilterDto, QueryContext queryContext = QueryContext.None, CancellationToken ct = default)
    {
        ...
        var retSeries = query.Select(SeriesMapping.ToSeriesDtoExpression(userId));

        return await PagedList<SeriesDto>.CreateAsync(retSeries, userParams.PageNumber, userParams.PageSize, ct);
    }

    public async Task<SeriesDetailRequestV3Dto?> GetLibrariannPlusSeriesDetailRequestV3Dto(int seriesId, CancellationToken ct = default)
    {
        ...
        CancellationToken ct = default)
    {
        ...
        var genrePreferences = await context.AppUserPreferences
            .Where(p => p.AppUserId == userId)
            .Select(p => new {p.FavoriteGenreIds, p.IgnoredGenreIds})
            .AsNoTracking()
            .SingleOrDefaultAsync(ct);

        var ignoredGenreIds = genrePreferences?.IgnoredGenreIds ?? [];
        var favoriteGenreIds = genrePreferences?.FavoriteGenreIds ?? [];

        if (ignoredGenreIds.Count > 0)
        {
            query = query.Where(series => !series.Metadata.Genres.Any(genre => ignoredGenreIds.Contains(genre.Id)));
        }

        var orderedQuery = favoriteGenreIds.Count > 0
            ? query.OrderByDescending(series => series.Metadata.Genres.Any(genre => favoriteGenreIds.Contains(genre.Id)))
                .ThenByDescending(series => series.Created)
            : query.OrderByDescending(series => series.Created);

        var retSeries = orderedQuery
            .Select(SeriesMapping.ToSeriesDtoExpression(userId))
            .AsSplitQuery()
            .AsNoTracking();
    ...
    public async Task<PagedList<SeriesDto>> GetRecentlyAddedInFavoriteGenresAsync(int userId,
        UserParams userParams, CancellationToken ct = default)
    {
        var query = await CreateFilteredSearchQueryableV2(userId, new SeriesFilterV2Dto(),
            QueryContext.Dashboard, ct: ct);
        var genrePreferences = await context.AppUserPreferences
            .Where(preferences => preferences.AppUserId == userId)
            .Select(preferences => new {preferences.FavoriteGenreIds, preferences.IgnoredGenreIds})
            .AsNoTracking()
            .SingleOrDefaultAsync(ct);
        var favoriteGenreIds = genrePreferences?.FavoriteGenreIds ?? [];
        var ignoredGenreIds = genrePreferences?.IgnoredGenreIds ?? [];

        query = favoriteGenreIds.Count == 0
            ? query.Where(_ => false)
            : query.Where(series => series.Metadata.Genres.Any(genre => favoriteGenreIds.Contains(genre.Id)));
        if (ignoredGenreIds.Count > 0)
            query = query.Where(series => !series.Metadata.Genres.Any(genre => ignoredGenreIds.Contains(genre.Id)));

        var result = query.OrderByDescending(series => series.Created)
            .Select(SeriesMapping.ToSeriesDtoExpression(userId))
            .AsSplitQuery()
            .AsNoTracking();
        return await PagedList<SeriesDto>.CreateAsync(result, userParams.PageNumber, userParams.PageSize, ct);
    }

    public async Task<PagedList<SeriesDto>> GetRecentlyDownloadedAsync(int userId, UserParams userParams,
        CancellationToken ct = default)
    {
        var accessibleSeries = await CreateFilteredSearchQueryableV2(userId, new SeriesFilterV2Dto(),
            QueryContext.Dashboard, ct: ct);
        var ignoredGenreIds = await context.AppUserPreferences
            .Where(preferences => preferences.AppUserId == userId)
            .Select(preferences => preferences.IgnoredGenreIds)
            .AsNoTracking()
            .SingleOrDefaultAsync(ct) ?? [];
        if (ignoredGenreIds.Count > 0)
        {
            accessibleSeries = accessibleSeries.Where(series =>
                !series.Metadata.Genres.Any(genre => ignoredGenreIds.Contains(genre.Id)));
        }

        var directSeriesImports =
            from download in context.AcquisitionDownloads
            where download.Status == AcquisitionDownloadStatus.Imported && download.ImportedSeriesId != null
            select new
            {
                SeriesId = download.ImportedSeriesId!.Value,
                ImportedAtUtc = download.ImportedAtUtc ?? download.CompletedAtUtc ?? download.CreatedAtUtc,
            };
        var wantedSeriesImports =
            from download in context.AcquisitionDownloads
            join wantedItem in context.WantedItems on download.WantedItemId equals wantedItem.Id
            where download.Status == AcquisitionDownloadStatus.Imported && wantedItem.LibrarySeriesId != null
            select new
            {
                SeriesId = wantedItem.LibrarySeriesId!.Value,
                ImportedAtUtc = download.ImportedAtUtc ?? download.CompletedAtUtc ?? download.CreatedAtUtc,
            };
        var monitoredSeriesImports =
            from download in context.AcquisitionDownloads
            join target in context.MonitoringTargets on download.MonitoringTargetId equals target.Id
            where download.Status == AcquisitionDownloadStatus.Imported && target.LibrarySeriesId != null
            select new
            {
                SeriesId = target.LibrarySeriesId!.Value,
                ImportedAtUtc = download.ImportedAtUtc ?? download.CompletedAtUtc ?? download.CreatedAtUtc,
            };
        var importedSeries = directSeriesImports.Concat(wantedSeriesImports).Concat(monitoredSeriesImports)
            .GroupBy(imported => imported.SeriesId)
            .Select(group => new {SeriesId = group.Key, ImportedAtUtc = group.Max(item => item.ImportedAtUtc)});

        var result = accessibleSeries
            .Join(importedSeries, series => series.Id, imported => imported.SeriesId,
                (series, imported) => new {Series = series, imported.ImportedAtUtc})
            .OrderByDescending(item => item.ImportedAtUtc)
            .ThenBy(item => item.Series.Name)
    ...
    public async Task<PagedList<SeriesDto>> GetNewFromFollowedAuthorsAsync(int userId, UserParams userParams,
        CancellationToken ct = default)
    {
        var query = await CreateFilteredSearchQueryableV2(userId, new SeriesFilterV2Dto(),
            QueryContext.Dashboard, ct: ct);
        ...
        query = query.Where(series => series.Metadata.People.Any(person =>
            person.Role == PersonRole.Writer && context.AppUserFollowedPeople.Any(follow =>
                follow.AppUserId == userId && follow.PersonId == person.PersonId)));
        if (ignoredGenreIds.Count > 0)
            query = query.Where(series => !series.Metadata.Genres.Any(genre => ignoredGenreIds.Contains(genre.Id)));

        var result = query.OrderByDescending(series => series.Created)
            .ThenBy(series => series.SortName)
            .Select(SeriesMapping.ToSeriesDtoExpression(userId))
            .AsSplitQuery()
            .AsNoTracking();
        return await PagedList<SeriesDto>.CreateAsync(result, userParams.PageNumber, userParams.PageSize, ct);
    ...
    public async Task<PagedList<SeriesDto>> GetNextInSeriesAsync(int userId, UserParams userParams,
        CancellationToken ct = default)
    {
        var accessibleSeries = await CreateFilteredSearchQueryableV2(userId, new SeriesFilterV2Dto(),
            QueryContext.Dashboard, ct: ct);
        ...
        var accessibleIds = accessibleSeries.Select(series => series.Id);
        var completedSeriesIds = context.Series
            .Where(series => accessibleIds.Contains(series.Id) && series.Pages > 0 &&
                             context.AppUserProgresses
                                 .Where(progress => progress.AppUserId == userId && progress.SeriesId == series.Id)
                                 .Sum(progress => progress.PagesRead) >= series.Pages)
            .Select(series => series.Id);
        var startedSeriesIds = context.AppUserProgresses
            .Where(progress => progress.AppUserId == userId && progress.PagesRead > 0)
            .Select(progress => progress.SeriesId);
        var nextSeriesIds = context.SeriesRelation
            .Where(relation => relation.RelationKind == RelationKind.Sequel &&
                               completedSeriesIds.Contains(relation.SeriesId) &&
                               accessibleIds.Contains(relation.TargetSeriesId) &&
                               !startedSeriesIds.Contains(relation.TargetSeriesId))
            .Select(relation => relation.TargetSeriesId);

        var result = accessibleSeries.Where(series => nextSeriesIds.Contains(series.Id))
            .OrderByDescending(series => series.Created)
            .ThenBy(series => series.SortName)
            .Select(SeriesMapping.ToSeriesDtoExpression(userId))
            .AsSplitQuery()
            .AsNoTracking();
    ...
    public async Task<PagedList<MissingSeriesItemDto>> GetMissingFromMonitoredSeriesAsync(int userId,
        UserParams userParams, CancellationToken ct = default)
    {
        var accessibleSeries = await CreateFilteredSearchQueryableV2(userId, new SeriesFilterV2Dto(),
            QueryContext.Dashboard, ct: ct);
        ...
        var query =
            from wantedItem in context.WantedItems.AsNoTracking()
            join target in context.MonitoringTargets.AsNoTracking()
                on wantedItem.MonitoringTargetId equals target.Id
            join sourceSeries in accessibleSeries on target.LibrarySeriesId equals sourceSeries.Id
            where target.IsEnabled && target.MonitorMissing && wantedItem.Status == WantedItemStatus.Missing
            orderby wantedItem.LastSeenAtUtc descending, sourceSeries.Name, wantedItem.Sequence
            select new MissingSeriesItemDto(
                wantedItem.Id,
                target.Id,
                sourceSeries.Id,
                sourceSeries.LibraryId,
                sourceSeries.Name,
                wantedItem.Title,
                wantedItem.Author,
                wantedItem.Series,
                wantedItem.Sequence,
                wantedItem.PublicationYear);

        return await PagedList<MissingSeriesItemDto>.CreateAsync(query, userParams.PageNumber, userParams.PageSize, ct);
    }

    public async Task<PagedList<SeriesDto>> GetBecauseYouReadAsync(int userId, UserParams userParams,
        CancellationToken ct = default)
    {
        var readSeriesIds = context.AppUserProgresses
            .Where(progress => progress.AppUserId == userId && progress.PagesRead > 0)
            .Select(progress => progress.SeriesId)
            .Distinct();
        var topGenreIds = await context.Series
            .Where(series => readSeriesIds.Contains(series.Id))
            .SelectMany(series => series.Metadata.Genres)
        ...
        var candidates = await CreateFilteredSearchQueryableV2(userId, new SeriesFilterV2Dto(),
            QueryContext.Dashboard, ct: ct);
        var ignoredGenreIds = await context.AppUserPreferences
            .Where(preferences => preferences.AppUserId == userId)
            .Select(preferences => preferences.IgnoredGenreIds)
        ...
        candidates = topGenreIds.Count == 0
            ? candidates.Where(_ => false)
            : candidates.Where(series => !readSeriesIds.Contains(series.Id) &&
                series.Metadata.Genres.Any(genre => topGenreIds.Contains(genre.Id)));
        if (ignoredGenreIds.Count > 0)
        {
            candidates = candidates.Where(series =>
                !series.Metadata.Genres.Any(genre => ignoredGenreIds.Contains(genre.Id)));
        }

        var result = candidates
            .OrderByDescending(series => series.Metadata.Genres.Count(genre => topGenreIds.Contains(genre.Id)))
            .ThenByDescending(series => series.Created)
            .Select(SeriesMapping.ToSeriesDtoExpression(userId))
            .AsSplitQuery()
            .AsNoTracking();
    ...
    public async Task<PagedList<SeriesDto>> GetOnDeckAsync(int userId, int libraryId, UserParams userParams, CancellationToken ct = default)
    {
        var settings = await context.ServerSetting
            ...
            .AsNoTracking()
            .ToListAsync(ct);
        var serverSettings = settings.ToServerSettingDto();
        ...
        CancellationToken ct = default)
    {
        ...
        return await context.Series
            ...
            .Where(s => seriesIds.Contains(s.Id) && allowedLibraries.Contains(s.LibraryId))
            .OrderBy(s => s.SortName.ToLower())
            .Select(SeriesMapping.ToSeriesDtoExpression(0)) // Matches the original unparameterized ProjectTo<SeriesDto>
            .AsNoTracking()
            .AsSplitQuery()
            .ToListAsync(ct);
    }
        ...
        SeriesIncludes includes = SeriesIncludes.None, CancellationToken ct = default)
    {
        ...
        return await context.Series
            ...
            .OrderByDescending(s => s.LowestFolderPath!.Length)
            .Includes(includes)
            .FirstOrDefaultAsync(ct);
    }
...
#nullable enable
    ...
    /// <summary>
    /// Returns the Average rating for all users within Librariann instance
    /// </summary>
    /// <param name="seriesId"></param>
    /// <param name="userId"></param>
    /// <param name="ct"></param>
