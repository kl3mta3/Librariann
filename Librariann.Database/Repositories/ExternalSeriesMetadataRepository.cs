...
public class ExternalSeriesMetadataRepository(DataContext context) : IExternalSeriesMetadataRepository
{
    ...
    public async Task<SeriesDetailPlusDto?> GetSeriesDetailPlusDto(int seriesId, CancellationToken ct = default)
    {
        ...
        var externalSeriesRecommendations = seriesDetailDto.ExternalRecommendations
            ...
            .Select(r => r.ToExternalSeriesDto())
            .ToList();
        ...
        var ownedSeries = await context.Series
            ...
            .Select(SeriesMapping.ToSeriesDtoExpression(0)) // No per-user context here; matches the original
            // unparameterized ProjectTo<SeriesDto>, which likewise never resolved a real userId for these fields.
            .ToListAsync(ct);
        ...
        CancellationToken ct = default)
    {
        var source =  context.Series
            ...
            .Select(SeriesMapping.ToManageMatchSeriesDtoExpression(context.Series));
