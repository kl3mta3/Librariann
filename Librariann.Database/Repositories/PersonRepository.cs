...
{
    public Task<AppUserFollowedPerson?> GetFollowAsync(int userId, int personId, CancellationToken ct = default) =>
        context.AppUserFollowedPeople.FirstOrDefaultAsync(follow =>
            follow.AppUserId == userId && follow.PersonId == personId, ct);

    public Task<bool> IsFollowingAsync(int userId, int personId, CancellationToken ct = default) =>
        context.AppUserFollowedPeople.AsNoTracking().AnyAsync(follow =>
            follow.AppUserId == userId && follow.PersonId == personId, ct);

    public void Follow(AppUserFollowedPerson follow) => context.AppUserFollowedPeople.Add(follow);

    public void Unfollow(AppUserFollowedPerson follow) => context.AppUserFollowedPeople.Remove(follow);
    ...
    public Task<Person?> GetPersonByOpenLibraryId(string openLibraryId, CancellationToken ct = default) =>
        context.Person.FirstOrDefaultAsync(person => person.OpenLibraryId == openLibraryId, ct);
        ...
        PersonRole role, CancellationToken ct = default)
    {
        ...
        return await context.ChapterPeople
            ...
            .Take(20)
            .Select(ChapterMapping.ToStandaloneChapterDtoExpression(userId))
            .ToListAsync(ct);
    }
